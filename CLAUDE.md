...
## Route before you code (main session included — direct edits are fine, doc drift is not)
0. Any behaviour change to `backend/` or `content/` **starts by reading** the owning `docs/plans/<subsystem>.md` + module `SPEC.md`. Unclear owner → search `docs/plans/`, not the code.
1. **Direct edits by the main session are allowed** (bug fixes, small changes) — but code and docs move together: a change already covered by the SPEC needs no ceremony; a change that moves the SPEC's contract updates that SPEC **in the same working set** as the code.
2. **A new mechanism is an Open-Question event, whoever holds the editor** — new state field, formula shape, cadence, schema key, or an item the plan explicitly deferred ("후행/보류"): enumerate options in the plan, surface via AskUserQuestion, and wait for the human RESOLVE. Code changes must always be aligned with — and for new mechanisms, **preceded by** — the Plan/SPEC updates; documentation is never retro-fitted or skipped.
3. After RESOLVE: plan (What) → SPEC (How) → implement (delegate or direct) → verify; the deliberation goes to `docs/decisions/` (Why). Intake details: `docs/process/spec-system.md` §Request intake.
