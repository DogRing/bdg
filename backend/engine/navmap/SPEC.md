...
// live in engine/climate; navmap is a passive writer here. Passing an unknown
...
- Ownership: `world` is the sole mutator (Deposit/Decay/StampFootprint/**SetTerrain** in the apply
  phase). `pathfind` receives a `Snapshot()` and must never mutate. `engine/climate` does **not**
...
- *When* to deposit/decay, *who* traverses, and *which cells transition + at what cadence* →
  ...
  list) → `engine/climate` (`backend/engine/climate/SPEC.md`). navmap only executes the write.
- Climate state (Moisture/Temperature), the rain model, and the from-type×condition→to-type
  transition table → `engine/climate` + `content/climate.yaml` (`docs/climate.md`).
