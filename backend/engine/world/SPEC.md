...
| [`SPEC-world-env.md`](SPEC-world-env.md) | **WI-P1** env orchestration: climate/flora/decay pure-Step driving + cadence · climate→navmap `SetTerrain` bridge · env sampling (flora `SiteInput` / decay env) · flora-shade→perception · env-OFF neutrality (`docs/plans/world-integration.md`) |
...
| [`SPEC-world-shelter.md`](SPEC-world-shelter.md) | **SH1–SH2**: shelter/exposure local-wind injection · `blocks_wind` blocker cache · cave portal/interior active-space state (`docs/plans/shelter.md`) |
