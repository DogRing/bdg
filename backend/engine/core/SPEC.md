...
- `Tick` and `GameMinutes` are plain `int64` time types. `core` fixes only the types; the
  conversion ratio between them lives in `engine/worldtime` (content-driven, D10). No
...
- `StatID`, `Dimension`, `Tag`, and `Pred` are uninterpreted strings at this layer; registries
  in `engine/stats`, `engine/needs`, `engine/actions`, and `engine/gates` give them semantics.
...
- Stat values and `StatRegistry` → `engine/stats`
- Need / value dimension definitions and their rates → `engine/needs` (which aliases
  `NeedID = core.Dimension`) + `content/needs.yaml`
- Spatial indexing and radius queries → `engine/spatial`
- Tick ↔ GameMinutes / hour / day / season conversion (and the ratio) → `engine/worldtime`
- Event stream storage, SSE, why-trace → `platform/events`
- Any GOAP or planning logic → `engine/planner`
