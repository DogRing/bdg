...
> `core.Tick` and `core.GameMinutes` are both `int64` types defined in `engine/core` (the
> `core` SPEC now declares both; see `backend/engine/core/SPEC.md`). `core` fixes only the
...
- `engine/core` — `Tick`, `GameMinutes` (the only imports; stdlib `errors`/`fmt` for `Validate`).
...
- Seasonal effects on resources / needs (e.g. regrowth, scarcity) → `content/balance.yaml`
  + the consuming module (`engine/needs`, resource regen in `engine/world`).
...
> Resolved: `core.GameMinutes` (and the `Tick` ownership question) — `engine/core` now declares
