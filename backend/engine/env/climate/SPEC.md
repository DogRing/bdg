...
## Dependencies
- `engine/kernel/core` — `Vec2`, `Tag` (the terrain id type). **Plus the shared §6 Formula (boolean
...
- `engine/kernel/rng` — `*RNG` (injected seeded fork; the rain process + duration draws, D12).
...
- *(NOT `engine/mind/gates`)* — climate must NOT import gates (gates is L2: depends on `stats`; climate is
...
- [ ] **No wall-clock / no global rand (D12 guard)** — grep guard: no `time` import for logic, no
  global `rand` call; the only randomness is the injected `*rng.RNG`. No `engine/space/navmap`, `worldtime`,
  or `gates` import.
...
- **§6 Formula-evaluator home module (P1 ESCALATION — blocks M2 implementation).** Climate `Rules`
  conditions are the **boolean subset** of the §6 DSL — the same subset `engine/mind/gates.GateExpr`
  already evaluates, which the glossary calls "one shared evaluator." Today that evaluator lives only
  in `engine/mind/gates` (L2, depends on `stats`), which climate (an L1 `core`+`rng` leaf) **cannot**
  import without breaking the DAG. Three options for the implementer/architect/human:
  - **(a) Lift the §6-DSL boolean evaluator to `engine/kernel/core` (L0)** so both `gates` and `climate`
