...
(used by `engine/mind/perception` and `engine/world` conflict/interaction checks). It stores
...
- `engine/kernel/core` — `Vec2` (`DistSq`/`Distance`), `ObjectID`. No other imports beyond `sort`.
...
- **No domain coupling**: the index knows nothing of agents, stats, or perception — only
  `(ID, Pos)`. Sense modeling (LoS, gradients) lives in `engine/mind/perception`.
...
- [ ] Determinism / brute-force oracle: for 1 000 random points (seeded `engine/kernel/rng`), the
...
- Sense modeling — line-of-sight, smell gradients, hearing falloff → `engine/mind/perception`
