...
consumes `engine/space/spatial` for proximity (D11: the spatial hash is the *only* source of position
...
// (NEW — flora 1d). It is the perception-facing projection of engine/env/flora.Shade: a position,
...
// imports engine/env/flora; the world adapts flora.State.ShadeOf into this projection (dependency
...
// inversion — perception does not import engine/world OR engine/env/flora). EntitiesInRadius is the
...
// (the spatial-hash contract, engine/space/spatial/SPEC.md) so this module's composition is stable.
...
> validation, and passes the reader/bytes here (same pattern as `engine/mind/values.Load`).
...
- `engine/kernel/core` — `Vec2` (`Distance`/`DistSq`), `ObjectID`, `Tag`. No IO.
- `engine/space/spatial` — `*spatial.SpatialHash` (via `spatial.New`) for proximity candidates and the
  LoS ray-march candidate set; the **only** source of position data (D11). Uses
  `NearbyEntities`/`NearbyIDs`/`PosOf` only.
- `engine/mind/actions` — `actions.ActionID` (carried on `SoundEvent`; perception does not interpret it).
...
- **Contract only — NOT imported**: `engine/world` implements `WorldSnapshot` (dependency
  ...
  `engine/env/flora.Shade`/`ShadeOf` into `ShadeOccluder` — perception **never imports `engine/env/flora`**
...
- **Spatial hash is the only position source (D11).** Positions (entities AND shade occluders) come
  from `engine/space/spatial` (via the `Sensor`'s index and `WorldSnapshot.EntitiesInRadius` /
...
- Position storage, radius-candidate gathering, bucketing → `engine/space/spatial` (this module consumes
...
- Deciding what to do with a percept — attention, fear, goal raising, gossip → `engine/agent` /
  `engine/mind/values` / `engine/mind/tom` (perception only reports what is perceivable, not its meaning).
- Action tag semantics (what `[loud]`/`[scented]`/`[opaque]` mean elsewhere) and tag-derived cost →
  `engine/mind/actions` / `engine/mind/gates` / `engine/mind/planner`; perception treats tags as opaque strings.
...
- **Position vs attributes split.** The spatial hash (`engine/space/spatial`) stores only `(ID, Pos)`. It
