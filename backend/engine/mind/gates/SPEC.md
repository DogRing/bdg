...
- `engine/kernel/core` — `StatID`, `Tag`, `ObjectID`. (Plus a YAML decoder and stdlib `sort`.)
- `engine/mind/stats` — `Stats`, `*Registry` (to reject a stat leaf naming an unknown `StatID` via
...
- **Decisions read `ToM[self]` for STAT leaves (D8)**: a stat leaf resolves against
  ...
  Stats — calibration is owned by `engine/mind/tom` (β). Underestimation stays self-sealing.
...
- **Tag-derived base cost** (`cost(action) = Σ planner.tag_costs[tag]`) and applying the
  `CostMultiplier` → `engine/mind/planner` (`backend/engine/mind/planner/SPEC.md`). This module *emits* the
...
- Deciding *whether to attempt* a visible action, goal arbitration, `Stickiness`/`Budget` →
  `engine/mind/planner`.
- Self-calibration of `ToM[self]` (β) and gossip → `engine/mind/tom`.
