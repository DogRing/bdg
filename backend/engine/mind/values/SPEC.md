...
-- the action's Effect / the consumed item's supply (engine/mind/actions, content/objects.yaml),
...
`ToM[target]`). The module keeps the `values → tom` edge one-way: it imports `engine/mind/tom` for the
...
- `engine/kernel/core` — `Dimension`, `Referent`, `Posture`, `StatID`. (Plus a YAML decoder and stdlib
  `sort` for `Load`.)
- `engine/mind/needs` — `Def` (reads `Threshold` as `maxIntensity`; `ComputeStanding` and
  `DeriveReferentInput` take a `needs.Def`). Per architecture §2 `values` depends on `needs`. No
  mutation of the need registry.
- `engine/mind/tom` — `Belief` (the `ToM[target]` shape, `= ToM[X]`, with `EstStats map[core.StatID]
  tom.StatDist`). **Per architecture §2/§4** `values → tom` is a ONE-WAY dependency: this module
  imports `engine/mind/tom` for the `tom.Belief` type and reads its `EstStats` means for the
  Other-referent derivation; it NEVER writes to `tom` and `tom` never imports `values`.
- `engine/mind/stats` — the stat `[Min,Max]` bounds used to normalize the Other-referent stat estimates
...
- **One-way `values → tom` (architecture §4)**: `DeriveReferentInput` READS `tom.Belief.EstStats`
  for the Other path and NEVER writes to `tom`; `engine/mind/tom` never imports `engine/mind/values`. A
...
- **Separation of want vs how (D5)**: `engine/mind/values` answers "how much is this wanted"; it never
  answers "how to get it" (no planning, no GOAP, no provisioning, no cost). That is `engine/mind/planner`.
...
- [ ] **`Load` reads `values.weights` from injected bytes (D10, path not hardcoded)**:
  ...
  numeric weight literal and no dimension-name literal in `engine/mind/values` source.
...
- [ ] **No hardcoded stat name (D7, P5)**: moodStatID is passed in; a stub with moodStatID="X" uses X.
  Grep guard: no "Mood"/"Intelligence"/"Social" literal in engine/mind/values source.
...
- **Selecting the goal dimension (arg-max over Priority) and the action (arg-max over EffValue),
  goal arbitration, `Stickiness`/`Budget`/`goal_deadband`, and any planning** → `engine/mind/planner`
...
- **Computing `ExpectedEffect` for a concrete action** (reading the action's `Effect`/the consumed
  item's supply, forward-simulating need satisfaction) → `engine/mind/planner` + `engine/mind/actions`
...
- **Per-agent need intensities/levels and their decay over time** → `engine/agent`/`engine/world`
  state (they feed `currentIntensity` in here) + `engine/mind/needs` forward-roll helpers.
...
- `currentIntensity` is the *grown* need (higher = worse), the complement of the satisfaction
  *level* used by `engine/mind/needs.Def.Level` (level = 1 − intensity-fraction). The caller converts
