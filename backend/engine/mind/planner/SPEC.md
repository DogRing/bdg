...
`engine/mind/values`), the planner assembles the shortest viable **ordered sequence of atomic actions**
...
// DimensionPriority is one row of the Priority-ordered goal list produced by engine/mind/values.
...
> `tom.Belief` is the self/other belief model the `engine/mind/tom` SPEC exposes (its `Belief` type,
...
- `engine/kernel/core` — `ActionID`, `Tag`, `Pred`, `Dimension`, `Vec2`, `ObjectID`, `Stats`, `Tick`.
- `engine/mind/stats` — `*Registry` (stat `[Min,Max]` for the Intelligence horizon; `IDs()` order),
  `Stats`.
- `engine/mind/gates` — `*Registry`, `Evaluate(Action, AgentSnapshot) Result`, `gates.AgentSnapshot`,
  `gates.Action`.
- `engine/mind/needs` — `*Registry`, `Def`, and the pure forward-roll helpers `Def.Demand`,
  `Def.BreachAt`, `Def.Level` (D9 forward-sim provisioning math lives in `needs`).
- `engine/mind/actions` — `*Registry`, `ActionDef`, `Producers(pred) []ActionID` (GOAP reverse index),
  `Effect`.
- `engine/mind/values` — `Priority`, `Salience` (the scalar types carried in `DimensionPriority`).
- `engine/mind/tom` — `Belief` (the `SelfModel` shape = `ToM[self]`). No `engine/mind/tom` change required.
- `engine/kernel/rng` — `*RNG` (injected, D12) for configured tie-breaking only.
...
via the `engine/mind/needs` helpers; iterate `needs.IDs()` order, D12. When `horizon_ticks == 0` (the
...
### Tag-derived cost (absorbed from `engine/mind/gates` schema_version 2)
...
### Dynamic visibility / conscience loosening (absorbed from `engine/mind/gates`)
...
- **Computing `Standing`/`Salience`/`Priority`/`EffValue`** → `engine/mind/values`.
- **Per-need decay, level roll, `Demand`/`BreachAt`, the conditional Safety driver** → `engine/mind/needs`.
- **Gate predicate evaluation** → `engine/mind/gates`; the planner only calls `Evaluate` + Urgency relax.
- **The action catalog, tags, `Producers` reverse index, `Effect`** → `engine/mind/actions`. The
...
- **`ToM[self]` construction, β self-calibration, gossip** → `engine/mind/tom`.
...
- **RNG type: `*rng.RNG` (RESOLVED).** Uses `engine/kernel/rng.*RNG`, not stdlib rand.
