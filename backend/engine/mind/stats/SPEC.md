...
// injected by platform/config, NEVER a file path in engine/mind/stats, keeping the engine
...
- `engine/kernel/core` — `StatID`. (Plus a YAML decoder, e.g. `gopkg.in/yaml.v3`, and stdlib `sort`.)
...
- **No IO**: `engine/mind/stats` imports no `os`/`net`/filesystem package; it reads from an injected
...
- [ ] No literal stat name (`"Strength"`, …) appears in `engine/mind/stats` source (grep guard, D7).
...
> own). Keeping it out prevents the reviewer from flagging an AC `engine/mind/stats` cannot prove.
...
- Sampling actual agent stat values at spawn (uses `GenSpec` + injected `engine/kernel/rng`) →
  `engine/agent` / `engine/world` generation.
- `ToM[self]` / `ToM[X]` belief vectors and self-calibration (β) → `engine/mind/tom` (they reuse the
  `Stats` shape but live there).
- Need/value dimensions (a separate registry) → `engine/mind/needs` + `content/needs.yaml`.
- Gate evaluation reading stats → `engine/mind/gates` + `content/gates.yaml`.

## Open Questions

None blocking. (The schema-validation responsibility split is now resolved: structural
validation in `platform/config`, semantic validation in `engine/mind/stats.Load`.)
