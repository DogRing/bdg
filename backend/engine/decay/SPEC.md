...
(D12). Mirrors `engine/flora`'s "pure read → return delta → world applies" shape exactly. Concept &
...
import (
    ...
    "github.com/dogring/bdg/backend/engine/expr" // Dm3(a): decay owns + evaluates the §6 accel Program (flora parity)
...
// OWNS the Program and evaluates it via engine/expr, flora parity). Built once by platform/config
// (parse the §6 accel via engine/expr, validate kind/item ids, enforce strictly-ascending thresholds).
// engine/decay evaluates it read-only; it never parses YAML (D10). Mirrors flora.Rules.
...
## Dependencies
- `engine/core` — `Tag`, `ObjectID`, `Dimension`. No IO.
- `engine/expr` — the shared §6 `Formula` evaluator (L0, already decided). decay OWNS the compiled
  ...
  compile step) and never imports `engine/gates`.
- `engine/rng` — `*RNG` (injected seeded fork; reserved for forward-compat, mirrors flora; discrete-
  threshold decay is deterministic and draws nothing in P_m2).
- *(NOT `engine/climate`)* — one-way: world samples climate `Moisture`/`Temperature` at the lot
...
- *(NOT `engine/world`)* — world owns objects[]/inventory; it APPLIES `StepDeltas` (age/transition/
  ...
  mutates world state.
- *(NOT `engine/flora`, `engine/navmap`, `engine/agent`)* — decay is an independent L1 leaf; it does not
...
- The **`Rules`** table is owned by `platform/config` (built from `content/objects.yaml` `decay:`
  blocks, with the `accel` §6 compiled via `engine/expr`), injected read-only; this module never parses
  or mutates it.
...
- [ ] **Env-coupled acceleration is multiplicative §6 (Q2/Dm2/Dm3)** — two lots differing only in
  ...
  evaluated via `engine/expr` (operands `temperature`/`moisture`), NOT hardcoded; `effectiveRate` is the
...
- [ ] **No wall-clock / no global rand / no forbidden import (D12 guard)** — grep guard: no `time`
  import for logic, no global `rand`; no `engine/climate`/`engine/world`/`engine/flora`/`engine/navmap`/
  `engine/agent`/`engine/gates` import (only `core`, `expr`, `rng`).
...
- The **Craft action** consuming decayed materials, and any value/planner reaction to decay state →
  `engine/actions` (P_m3) + `engine/values`/`engine/planner`.
...
- Reference paths: `docs/materials.md` (Q1–Q5 + Dm1–Dm5 all RESOLVED), `docs/design.md §5/§9`,
  `backend/engine/flora/SPEC.md` (the structural template), `backend/engine/climate/SPEC.md` (the env
  output shape decay consumes as a value), `backend/engine/expr/SPEC.md` (the §6 evaluator decay owns +
