	...
	for _, a := range obj.Fauna.Actions {
		...
		if def, ok := actReg.Get(id); ok {
			for _, tag := range def.Tags {
				switch tag {
				case fauna.TagSteerFood, fauna.TagSteerWater, fauna.TagSteerPrey, fauna.TagSteerMate, fauna.TagSteerCover, fauna.TagFleePred, fauna.TagWaryPred, fauna.TagNoLoco, fauna.TagSleep, fauna.TagAttack, fauna.TagFeed:
					steer[id] = tag
				}
			}
		}
