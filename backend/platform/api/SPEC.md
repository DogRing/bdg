...
| `GET /api/flora` | `rds.Get(keyer.Flora())` — forwards the stored `sim:{run}:flora` bytes **verbatim** (WI-P4, data-contracts §2; `persist.FloraDoc` is `{world_revision, stream_cursor, flora:[{object_id, species, pos, stage, width}]}`). `world_revision` prevents cross-world mixing; `stream_cursor` identifies the baseline's as-of point so the client accepts a gap-recovery baseline only when it is at least the fresh snapshot cursor and replays only later buffered ops. Installed-empty is `200` with `flora:[]`; flora-off is `404`. Registered on `New` only, not `NewSSE`. | `200` + flora JSON · `404` + `{"error":"flora not found"}` when the key is absent/empty (flora OFF) |
...
- [ ] `GET /api/flora` returns the `sim:{run}:flora` bytes **verbatim** with `200` when the key
  exists (WI-P4) — the `FloraDoc` `{world_revision, stream_cursor, flora:[{object_id, species, pos, stage,
