...
| Thermal comfort band | `comfort_temp` + `thermal_band` | The ONE shape for a living thing's temperature response, shared by fauna and flora: `clamp01(\|T − comfort_temp\| / thermal_band)` — symmetric, so cold AND heat both stress. `thermal_band ≤ 0` = neutral (no response authored). Fauna feeds it to the `thermal` **drive** from `apparent_temp` (FA5/F40); flora exposes it as the `thermal_stress` **§6 operand** from `temperature` and clamps in the operand, since suitability terms are weighted summands (1l). Polarity is identical: 0 = comfortable, 1 = maximally stressed. |
| Thermal stress | `thermal_stress` (§6) | Flora's derived comfort-band operand ∈[0,1] (1l). Content spells a species' temperature response as a `(1 - thermal_stress)` summand; raw `temperature` arithmetic pretending to be normalized is the bug this replaced (`docs/decisions/flora-thermal-comfort.md`). Per-species: the same site stresses two species differently. |
