...
- `engine/agent/`, `engine/world/` — top-of-DAG controller + orchestrator, left **flat**. The future
  `engine/fauna/` (reduced-reactive animal controller, `docs/plans/fauna.md`) lands flat next to `agent`.
...
| `fauna` | **Reduced-reactive animal controller** (`docs/plans/fauna.md`; flat next to `agent`): per-tick **horizon-1 utility arbitration** over the shared atomic-action registry (drives + §6, **no planner/ToM**); **reads** the shared `space/scent` field (world deposits from `scent:<channel>` emitters). Emits intents; **`world` applies** (combined agent+animal ID order — dependency-inverted like climate/flora/decay). Does **NOT** import `world`/`agent`/`planner`. | core, expr, rng, actions, spatial, space/scent | L4 |
...
- `backend/cmd/sse` → **bdg-sse** (`sse.dogring.kr`): read-only `api.NewSSE` server tailing the valkey event stream with a read-only user. Stateless, scalable. Built/shipped separately (`backend/Dockerfile.sse`, `.github/workflows/sse.yaml`). See `docs/ops/deployment.md`.
...
- `objects.yaml` — object_kinds + item_kinds carrying their **supply** Effect (D9). Placement/counts
  ...
  **FINITE SOURCES** (`ore_node`) carry a `source: { initial, depleted_terrain }` block (object-local
  ...
  into `flora.Rules`/`decay.Rules` (`docs/plans/flora.md`, `docs/plans/materials.md`).
- `recipes.yaml` *(new, see `docs/plans/materials.md §0 "Recipe model — FINAL"`, `content/schema/recipes.schema.json`)*
...
- `terrain.yaml` *(planned, see `docs/plans/map.md`)* — **바탕재료 = 속성 프리셋**(`grainSize·moisture·temperature·depth·slope·salinity`); cost·passability는 per-type 상수가 아니라 **속성 위 §6 수식**(`docs/core/design.md §5`) + the
...
- `climate.yaml` *(see `docs/plans/climate.md`, `content/schema/climate.schema.json`)* — the **dynamic-terrain
