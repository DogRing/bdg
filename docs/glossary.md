...
| Terrain type | base material | `soil`/`sand`/`river`/`mountain`/`sea`(+`bare_rock`): a `navmap.TerrainType` (`BaseCost`/`Passable`/`RequiredTags`) + the §5 attribute preset. `content/terrain.yaml`. (Water = high-`BaseCost` traversable for fauna, not `!Passable`, R2.) |
