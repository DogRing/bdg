...
| Nav cost field | `navmap.NavMap` | Grid-indexed traversal cost + footprints + `wear` + dynamic terrain; `SetTerrain` (climate transition write) + `TerrainOverrides` (sparse delta). `engine/space/navmap`. |
