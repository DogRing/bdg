...
> **성격:** `docs/plans/fauna.md`의 Open-Question **심의 전문**(옵션 (a)(b)(c)·기각 근거·⚠가드·유도 SPEC-design).
> 확정값(What)은 plan의 resolution 테이블이 권위, 구현(How)은 `backend/engine/fauna/SPEC.md` 등 module SPEC.
> 이 파일 = 2026-07-09 다이어트 때 plan에서 잘라낸 **pre-diet 원문 스냅샷**(§1.3~클러스터 10; 이후 갱신되지 않음).
> F1~F24의 pre-resolution 옵션 전문은 이 정책 이전에 이미 압축됨 → 커밋 `bebc643`.
