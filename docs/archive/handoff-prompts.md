...
>
> **Decisions are accepted** (see `docs/plans/activation-gate.md` — G1–G17 = the recommendations). The agent
...
```
Implement the fixture loader + main wiring that ACTIVATES the ecosystem, per docs/plans/world-gen.md +
content/schema/fixture.schema.json + the InstallEnv/InstallFauna signatures (WI-P1/P2). Read HANDOFF.md +
docs/plans/activation-gate.md (decisions accepted). Build/extend tools/worldgen Load: fixture → terrain layout →
...
```
Implement env-state serialization + the REAL WorldFrame SSE projection per docs/core/data-contracts.md §4/§10.
...
docs/plans/scenarios-world.md (FA1–FA8) + docs/plans/activation-gate.md. The engine + wiring are built; this is the
...
- **Each phase flips the matching `docs/plans/activation-gate.md` Open-Question entries to RESOLVED** as it is
