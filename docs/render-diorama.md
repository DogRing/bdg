...
Concept & rationale: this file. Sits beside `docs/frontend-plan.md` (that plan built the flat top-down
viewer; this one restyles the **rendered** scene into a 2.5D billboard diorama for aesthetics) and
`docs/hex-grid.md` (a parallel visual migration of the same terrain layer — see §6 coordination). It is
...
> of this feeds back into state. `docs/data-contracts.md` does not change. `docs/design.md` does not
...
- **DL1 — Presentation transform only.** 2.5D is a projection + draw-order change in the render layer.
  ...
  new SSE field, no `/api/*` change, no `docs/data-contracts.md` edit.
...
- **terrain.ts:** **square** raster — `makeTerrainRaster` paints 1px/cell over `grid.w×grid.h`,
  ...
  but the **code + type are still square** — the hex migration (`docs/hex-grid.md`) is not yet in the
...
- Cross-links: `docs/frontend-plan.md` (pointer), `docs/hex-grid.md` (coordination note, Q8),
  `docs/glossary.md` (billboard / facing / depth-sort / ground-shadow vocabulary).
- **Not touched:** `docs/data-contracts.md`, any `backend/**`, `docs/design.md` (stated for the record).
...
- **Coordination with the hex migration (`docs/hex-grid.md`).** Both restyle the **same terrain layer**
...
## 7. Build order (slots into `docs/architecture.md §5`, frontend `L10`)
