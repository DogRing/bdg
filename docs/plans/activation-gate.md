...
- **Source:** `backend/engine/env/flora/SPEC.md` §Open Questions; `docs/plans/flora.md` 1e.
...
- **Source:** `backend/engine/env/climate/SPEC.md` §Open Questions ("Cross-module °C re-baseline");
  `docs/plans/world-integration.md §3`. (Schemas already updated to CA1-3; this is the DATA threshold pass.)
...
- **Source:** `engine/world/SPEC-world-fauna.md` + `SPEC-world-env.md` (the `InstallEnv`/`InstallFauna`
  + fixture placement); `docs/plans/world-integration.md` W11 (population). The starter fixture already places
...
- **Source:** `backend/engine/fauna/SPEC.md` §Out of Scope (climate operand source + apparent_temp);
  `docs/plans/climate.md §1c` CA1–CA3.
...
| G16 | `wind.dir`/`wind.mag` glossary coin | climate SPEC | register in `docs/core/glossary.md` (naming only) |
