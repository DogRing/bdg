...
- **신설 가드:** ①**영양 서열 트립와이어** — 포식자가 피식자보다 **크게** 더 굶주리면(>0.15) 실패. 비율 목표가 아니라 **구조적 단언**이고, 재교정 이전 상태(0.657 vs 0.355)를 정확히 잡는다(뮤테이션 검증). 기존 개수 단언들은 이 병증을 **하나도** 못 잡았다(양측이 형식상 공존했으므로). ②**다시드 가드** `TestPredatorBalanceAcrossSeeds` — 단일 시드 편차가 크므로(같은 밸런스에서 포식 1~26) 한 시드에 맞춰 튜닝하는 과적합을 차단.
