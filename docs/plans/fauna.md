...
형제 문서(양식·패턴 출처): `docs/plans/materials.md`(FINAL recipe model — 산물이 *이미 설계된* craft 사슬로 합류),
`docs/plans/flora.md`(순수-Step transform·다중주기·종=`objects.yaml` 블록·outcome-중립 staging),
`docs/plans/climate.md`(체감온도·바람 입력 계약·결정성 패턴), `docs/plans/map.md`(navmap/직렬화/보조-색인 양식).
...
> 패스에 먹인다(`docs/plans/climate.md §1c` 통합 seam). `wind.dir` 단위(라디안 등, CA2)·`wind.mag` 강도가 스텐실 전파 방향·범위를 정한다.
...
> 노출**(`docs/plans/climate.md §1c` CA2 바람 생성·CA3 단위/노출). 특히 **CA3 단위 fork**(정규화 [0,1] vs °C) 는 이 수식이 무엇을 읽는지를
...
> 양쪽에 주입한다 — `docs/plans/climate.md §1c` 통합 seam 의 fauna 측. climate `Wind` 생성은 climate step cadence(CA2), 소비는 여기.
...
*M5 — 매복·감지(ambush/detection). **RESOLVED 2026-07-02 (추천대로) → SPEC 착수 대기.** predator가 cover 은신으로 prey 시야(Flee)를 늦춰 접근 → 매복 창발.*
- **사실 확인(2026-07-02):** fauna 감지는 **반경+FOV**(`sightQuery`: SightRadius/fovArc + spatial NearbyEntities)이며 **shade/LoS 미반영**(perception full-LoS 동물 지각 = park). ⇒ shade(어두운 숲)로 매복이 자동 창발하지 않음 → M5-b가 이를 cover-근접 피탐지↓로 근사.
- **M5-a prey 조기감지 튜닝 — RESOLVED: 지금 적용(content-only).** prey `senses.sight_radius`/`fov_arc` 튜닝(기존 필드) + M5-b `conceal_factor`가 predator 은밀성 레버. 신규 메커니즘 아님(D10).
- **M5-b 매복 메커니즘 — RESOLVED: (ii) cover-근접 피탐지 반경 축소.** predator가 cover flora 밀도 높은 곳이면 prey의 sight 탐지 반경↓ → 냄새(Wary)로는 알아도 시야(Flee)가 늦어 접근 허용 → 매복. (i) shade/LoS occluder는 후행 park.
- **M5-c 몰이(cornering) — RESOLVED: 별도 코드 없음(창발 유지).** 강+bounds+M4 지형비용의 위치 상호작용으로 자연 창발(D2/D3).

> **SPEC design (2026-07-02, 게이트 resolve로부터 유도 — 발명 아님; M3(`HiddenUntil`)+M4-b(`coverDensity`) 재사용).** **단일 writer=world.** ① M4-b `coverResistance`에서 종-무관 `coverDensity(p)`(cover flora peaky 겹침 합) 추출(`coverResistance = 1 + coverDensity(p)×CoverCost`, M4-b 거동 불변). ② world `commitAnimalOwnState`가 이동 커밋 후 `a.Concealment = coverDensity(pos)×conceal_factor`를 씀(종 무관, ≥0; scent/`HiddenUntil`처럼 post-move 위치·1틱 지연 F33). `Animal.Concealment`는 **파생 transient**(매틱 재계산·비영속, 복원 시 1틱 내 self-heal). ③ fauna `sightQuery`: predator 후보를 `dist > SightRadius/(1 + other.Concealment)`이면 skip(은폐 predator는 유효 시야 반경↓; M4-b `1+density×cc`와 동형 포화식). **scent(Wary)·`combatTarget`(hunt/M3 hidden)은 불변 — 시야(Flee)만 늦춤**(prey는 냄새로는 알되 늦게 봄). **신규 Snapshot seam 없음**(M3 `other.HiddenUntil` 읽기와 동형으로 `snap.Animals[idx].Concealment` 읽음), 신규 §6 operand 없음. content/config: `world.yaml cadence.conceal_factor`(float ≥0) + `world.schema.json` + `EnvConfig.FaunaCombat.ConcealFactor`(`CombatParams`); M5-a는 prey `senses` 튜닝(기존 필드, 신규 필드 없음). **OFF-neutral:** `conceal_factor=0` ∨ cover flora 없음 → `Concealment≡0` → effRadius≡SightRadius → 시야 byte-identical(골든 불변). RNG 없음(D12).

*M6 — 관성·저크(inertia/momentum/juke). [후행 phase — M3~M5 착지 완료 2026-07-02, unblocked]. **사실 확인:** 현행 이동엔 관성 없음 — `steerFull`이 매 틱 desired heading으로 즉시 회전(`nextHeading = desired + wander`)·full effective speed 이동 → prey/predator 모두 180° 즉시 반전 가능. M6 = heading(±속도) 변화율 제한 → prey는 더 급회전(juke), predator는 못 꺾어 오버슈트. **주의: ~15% 포식 목표는 M1~M5로 이미 달성** — M6은 리얼리즘/창발 품질 향상이지 타겟 튜닝이 아님(원하면 defer 가능).*
- **M6-a 메커니즘·위치 — RESOLVED: (i) fauna §6 turn_rate 캡.** `steerFull`에서 `nextHeading`을 `a.Heading` 기준 `±turn_rate×DT`로 클램프. 기존 `a.Heading` 상태 재사용, **신규 필드/직렬화/world 변경 없음**. 비대칭 turn_rate만으로 juke+오버슈트 동시 창발(predator heading이 천천히 돌아 juking prey를 지나침). (ii) world velocity 관성 / (iii) 둘 다는 기각.
- **M6-b 레버 형태 — RESOLVED: (i) §6 `Program`.** turn_rate = base+Agility 파생(Speed/HideChance 동형). D7 정합 — 회전 민첩성은 competence라 stat 합성으로 창발('nimble' emerge). (ii) 스칼라 content는 기각.
- **M6-c 범위 — RESOLVED: (i) heading turn-rate만.** 회전 제한만으로 juke/오버슈트(최소 표면). speed `accel` 캡(ii)은 후행 증분으로 보류.
...
- **glossary(F19/F42):** §1.2 + §1.3 F42 용어(+ F43/F44 의 `Wary`/`sight.predator`/`dist.predator`/`smell`/`sight`/`fov_arc`, cross-doc `wind.mag`) 채택 확정 → `docs/core/glossary.md` 동기화는 별도 단계.
...
- **cross-doc(climate, 2026-06-26):** F40(apparent_temp)·F33(scent 바람 확산)·F41(world 와이어링)이 `docs/plans/climate.md §1c`
