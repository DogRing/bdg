...
- **RESOLVED: (B) 정적=묶음 확산·영속 / 동적=매 틱 확산 (사람 승인 2026-07-19) → SHIPPED.** `scent.Grid`가 두 층(`static`/`committed`)을 갖고 읽기(`IntensityAt`/`Read`)가 **두 층의 합**을 반환. 동물=`Deposit`+`Spread`+`Commit` 매 틱, flora/objects=`DepositStatic`+`CommitStatic` 묶음 주기(그 사이 **유지**). 확산 커널은 `spreadInto(buf,wind)`로 공유해 두 층이 산술적으로 어긋날 수 없음. **검증:** 식물 위 food 냄새가 전 틱 7.820 유지(재구축 틱에만 7.941로 갱신), 동물 prey 1.280 전 틱 유지 — 이전엔 정적 층이 6틱 중 5틱 0.000.
