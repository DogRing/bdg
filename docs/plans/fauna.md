...
### 클러스터 8b — 냄새장 2층화 (정적/동적 분리) — **OPEN (2026-07-19 제기, 사람 RESOLVE 필요)**
> **원래 의도(사람, 2026-07-19):** "타일에 있는 존재가 변하면 그 타일 냄새가 변한다. flora는 타일 변화가 없으니 **바람 확산만** 묶음 주기." → 즉 냄새장은 **점유(occupancy) 파생 영속 장**이고, `scent_spread` 주기는 *확산 전용*이어야 한다.
> **실제 코드는 그렇지 않다(실측 2026-07-19).** `Grid.Commit`이 매 틱 pending↔committed를 교체하며 **옛 버퍼를 통째로 비워** 아무것도 유지되지 않는다. 그래서 침전 주기가 곧 존재 주기가 된다.
- **1차 수정 SHIPPED (커밋 4cdc628):** 동물은 **움직이는 점유자**라 타일이 매 틱 실제로 바뀜 ⇒ 전 채널 매 틱 침전. **동적 층은 이제 의도대로 동작**(실측: prey 냄새 존재율 16.7%→99.9%, 포식자 사냥 시간 ~0%→6.6%).
- **⚠️ 남은 절반 = 정적 층.** flora/objects는 여전히 묶음 주기 침전이라 **food 냄새가 6틱 중 1틱만 존재**(실측: 식물 위에서 mod6==1일 때 4.939, 나머지 5틱 0.000). **초식동물의 방향성 먹이 탐색이 5/6 장님** — 풀이 사방에 있어 굶지 않을 뿐, 의도된 동작이 아니다. P_fa4a `scent_acuity` 튜닝값도 이 장 위에서 측정된 것이라 재측정 대상.
- **RESOLVED: (B) 정적=묶음 확산·영속 / 동적=매 틱 확산 (사람 승인 2026-07-19) → 영속 절반만 SHIPPED (커밋 064c930).** `scent.Grid`가 두 층(`static`/`committed`)을 갖고 읽기(`IntensityAt`/`Read`)가 **두 층의 합**을 반환. 동물=`Deposit`+`Spread`+`Commit` 매 틱, flora/objects=`DepositStatic`+`CommitStatic` 묶음 주기(그 사이 **유지**). **검증:** 식물 위 food 냄새가 전 틱 7.820 유지(재구축 틱에만 7.941로 갱신), 동물 prey 1.280 전 틱 유지 — 이전엔 정적 층이 6틱 중 5틱 0.000.
- **🔴 결함: 정적 층의 *확산*은 착지하지 않았다 (감사 2026-07-20, 미수정).** 공유 커널 `spreadInto(buf,wind)`가 `buf`에서 **읽고** 결과는 언제나 **동적 버퍼에 쓴다**. `Spread()`는 `buf==pending`이라 우연히 맞고, `CommitStatic()`은 어긋난다. 실측(`DepositStatic(food,(0.5,0.5),10)` + `CommitStatic`): `static={(0,0):10}` **halo 0**, `pending=`이웃 8칸 각 0.3(`10×0.40×0.60÷8`). 결과 셋:
  ① **정적 층은 한 번도 확산되지 않는다** ⇒ 기각했던 **옵션 (C)가 사실상 출하**된 상태다(플랜·SPEC·커밋 본문은 셋 다 (B)라고 말한다). food 채널은 **전량 정적**(flora 6종 전부 `scent:food`)이고 carrion도 정적이라, 식물/사체 냄새가 자기 칸을 벗어날 통로가 아예 없다.
  ② **묶음 주기마다 동적 층에 유령 halo가 주입**된다(`CommitStatic`이 `Commit` 뒤에 불리므로 비워진 pending에 떨어져 다음 틱 committed에 실렸다가 사라짐) ⇒ 8b가 없애려던 **6틱 중 1틱 깜빡임이 peak에서 halo로 자리만 옮겨** 남아 있다.
  ③ **질량 비보존** — 감산도 pending에 적용돼 0에서 클램프되므로 정적 층은 전량 유지하고 halo는 그 위에 새로 생긴다(재구축 틱마다 +24%).
  **왜 안 잡혔나:** 064c930이 신규 public API 2개를 **단위 테스트 0개**로 출하했고(지금도 `DepositStatic`/`CommitStatic` 호출 테스트가 저장소에 없다), SPEC은 그 커밋이 아니라 한 기능 뒤(e3f37fc)에 쓰였다. 결함이 **결정적**이라 골든은 전부 재현하며 통과한다 — 골든은 거동을 고정하지 의도를 고정하지 않는다.
  **처리:** 메커니즘 선택이 아니라 (B)를 실제로 구현하는 **결함 수정**이므로 **신규 게이트 없음**. 커널이 읽은 버퍼에 쓰도록 고치고, 정적/잔향 층에 없던 인수 기준을 SPEC에 추가한다. 재기준 범위·기각 대안(=(C)로 재확정) 전문 = **`docs/decisions/scent-static-layer-diffusion.md`**. 구현 정본 = `engine/space/scent/SPEC.md` §Static layer·§Invariants(layer isolation)·§Acceptance Criteria.
  **⚠ 수정은 단독 착지 + before/after 실측**(PD11 선행 경고와 동형): 무효화되는 기준선 = 전 scent 골든 + flora/carcass 포함 world 골든 · PD9 `strength`(0.010) · P_fa4a `scent_acuity`(8b가 이미 재측정 대상으로 지목했고 아직 재측정 안 됨) · PD4-v `graze_depletion_per_hunger`(5.0)와 클러스터 10 재적합.
- **⚠️ 후속 = 밸런스 재튜닝(별도 단계, 사람 결정 2026-07-19).** 예고대로 밸런스가 재기준됐다: **arena 사냥 성공률 ~44%→64.1%**, 밸런스 베드 24.5%→39.2%. 포식자가 6틱 중 5틱 장님이던 상태가 해소되며 추적력이 크게 올랐기 때문. **arena 상한 가드(≥50%)는 삭제가 아니라 PARK**(`fauna_arena_test.go`) — 그 임계와 "~11–12.5% 밴드"는 **결함 있는 냄새장 위에서 교정된 값**이라 유효 기준선이 아니고, 실제로 arena 피식자 개체수는 11→20으로 **증가**해 "학살" 상황이 아니다. 나머지 4개 가드(아무도 사냥 안 함/아무도 안 죽음/M3 미발동/피식자 멸종)는 **live 유지**. ⚠️ 튜닝은 arena(교전 표본 30여 건, 1건당 ~3%p 진동)가 아니라 **밸런스 베드(124건)+`balance.yaml`/`tools/tuner`** 로 할 것. 이번 세션의 단발 노브 시도 4회(피식자 속도↑·전종 acuity↓·포식자만 acuity↓)는 전부 미수렴이거나 부작용(초식 아사 30→74, 은신 1.33%→0.08%)이라 **전부 되돌림**.
- [RESOLVED 참고] **2층 구조의 형태 옵션** — 공통: 읽기 = `static + dynamic` 합, 동적 층 = 매 틱 재구축, 정적 층 = flora 변화 시에만 재구축(=비우지 않음). **쟁점은 바람 확산이 어느 층에 어떤 주기로 걸리는가**(정적 층을 영속시키면 그 *확산된 형태*도 영속해야 하는데, 바람은 CA2 랜덤워크로 계속 변한다). *options:* **(A) 두 층 모두 묶음 주기 확산 + 영속** — 재구축 빈도가 현행과 동일해 **비용 증가 0**이고 바람 stale 허용폭도 현행과 같음(Ns틱). 가장 작은 충실 구현. 단 동적 층은 매 틱 원시 점 침전으로 덮이므로 확산 효과가 1틱만 유지. **(B) 정적=묶음 확산·영속 / 동적=매 틱 확산** — 동물은 수십~수백이라 매 틱 확산도 저렴하고 prey 냄새 plume이 항상 살아 있음(포식 리얼리즘에 유리). 비용 소폭 증가. **(C) 정적은 확산하지 않음** — 식물 plume은 자체 형상으로 두고 바람은 동적 층에만. 가장 싸지만 "바람에 실려오는 풀냄새"가 사라짐. **rec: (B)** — 의도의 "바람 확산은 묶음 주기"는 *비싼 정적 층*에 대한 것이었고, 동적 층은 애초에 매 틱 갱신이 전제라 확산도 매 틱이 자연스럽다. ⚠️ 어느 선택이든 **골든 재기준**(냄새장 값이 전 구간 변함) + P_fa4a·클러스터 10 밸런스 재측정이 따라온다.
- **정직 플래그:** 이 항목은 2026-07-19에 코드 주석·커밋 메시지에만 "별도 과제"로 적히고 **플랜에 게이트로 등재되지 않았다**(절차 누락). 여기 등재해 바로잡음.
...
>
> **⚠️ "~15%"는 참조점이지 노브를 몰아갈 목표가 아니다 (사람 확정 2026-07-19).** 사냥 성공률은 **조건의 결과**다 —
> 피식자 밀도·상태·엄폐·지형에 따라 같은 콘텐츠가 베드마다 전혀 다르게 읽힌다(**피식자가 많아지면 사냥률은 올라간다**).
> 한 픽스처에서 15%를 강제하면 그 픽스처의 stocking을 밸런스 상수에 새겨 넣는 것이다. 베드는 **비율을 관측**하고
> **공존을 단언**한다(`TestPredatorBalanceObservation` + 다시드 `TestPredatorBalanceAcrossSeeds`).

#### 밸런스 재교정 (2026-07-19, 급식 루프 픽스 `7ce6edf` 직후)
**모든 튜닝 상수가 고장난 급식 루프 위에서 맞춰져 있었으므로 재적합이 필요했다.** 진단은 노브를 만지기 전에 **사냥 깔때기 계측**으로 시작했고, 그게 원인을 바꿔놨다:
- **증상:** 포식 베드가 "아사 86 vs 포식 1" — 포식자-피식자 계가 아니라 기근이었다. 포식자는 배고픈데(0.657) 피식자는 배불렀다(0.355).
- **깔때기:** 포식자 기준 피식자 30u 이내 **85%** → 10u 이내 **11%** → 교전 **1.1%**. 접근이 끊기는 지점은 "가까이 있음"과 "닿음" 사이. 급식 픽스 전후 비교로 귀속: 10u 이내가 **30.7%→11.0%**로 붕괴했고, 피식자 이동량이 **0.1175→0.0760**으로 급감했다.
- **인과:** 급식이 고쳐지자 피식자가 포만(58%)해졌고, **FM14b hunger-restlessness가 유일한 이동 동력**이라 배부른 피식자는 `move_deadband` 아래로 떨어져 **정지·군집**한다 ⇒ 포식자가 마주칠 일이 사라진다. 즉 **"피식자 포만"과 "포식자 생존"이 구조적으로 상충**한다(이 엔진에선 조우가 피식자 허기로 구동되므로).
- **RESOLVED: 먹이 희소성 재적합** — `graze_depletion_per_hunger` **2.0 → 5.0**. k는 한 끼의 영양가가 아니라 **한 패치가 몇 끼를 버티는가**를 정한다(`recovery = removed/k = want`, 생물량이 남아 있는 한 불변) ⇒ 국소 고갈을 앞당겨 이동압을 만든다. **5시드 평균**(포식 베드 6000틱): 포식 **4.2→10.4** · 포식자 아사 **17.8→12.0** · 포식자 잔존 **5.6→6.4** · 피식자 아사 57→187 · 피식자 개체수 유지(min 33→28, final 43→38). **피식자 사망 원인이 기아 일변도에서 포식으로 일부 이동** = 의도한 방향. k 스윕은 4~7이 고원(차이는 잡음)이라 피식자 아사가 가장 낮은 고원 초입을 택함.
- **동반 보정:** 전역 k는 **가장 덜 움직이는 종을 먼저 무너뜨린다** — 정주성인 물고기는 국소 수초가 바닥나도 벗어나지 못해 학교가 11.8→5.9(리스폰 하한)로 붕괴했다. `waterweed.length_rate` **0.10→0.30**으로 13.0 회복(최소 보정치; 민감도 0.10→5.9 · 0.30→13.0 · 0.60→19.8 · 1.00→33.0).
- **기각한 대안 — 포식자 후각 확대**(wolf 20→32 / bear 22→34). 가설은 "감각을 늘려 *피식자 허기*와 *포식자 생존*의 결합을 끊는다"였으나 **2×2 요인실험(5시드)에서 실패**: k=5에서 사냥 10.4→11.8로 소폭 늘지만 **포식자 아사가 12.0→14.2로 악화**. 희소성이 지배 레버이고 감각은 결합을 못 끊는다 ⇒ 저작값 유지. (`sight.prey` 오퍼랜드 신설은 **신규 메커니즘 = 게이트 사안**이라 시도하지 않음.)
#### PD8 — 라이브 월드에서 포식자가 무리를 찾지 못함 — **RESOLVED: 밀도 상향 + 굶주림 시계 (사람 승인 2026-07-19)**
**위 재교정은 잘못된 베드에서 이뤄졌다.** 사람 지적("포식자도 피식자가 모여 있으면 그 주변에 있는 것이 맞다")을 계기로 **배포 월드(`rabbit_meadow`)를 직접 계측**하니, 손으로 저작한 포식 베드에는 없는 병증이 있었다:

| | 포식 베드(튜닝 대상) | **rabbit_meadow (배포본)** |
|---|---|---|
| 피식자 30u 이내 | 88% | **2.0%** |
| 100~200u 떨어짐 | 0.4% | **76.3%** |
| 6000틱 사냥 | 11 | **0** |

- **원인은 행동이 아니라 산수다.** 못 먹은 포식자는 **약 1게임일**(1440틱)에 아사하고 틱당 ~0.2 units 이동 ⇒ **평생 탐색 예산 ≈ 280 units**. 그런데 500² 맵에서 피식자는 100~200u 떨어져 **뭉쳐** 있고 냄새는 ~20u까지만 간다. 찾아갈 여력이 원천적으로 없었다. (배부른 피식자가 정지·군집하는 것 자체는 **정상**이다 — 문제는 포식자가 그 무리에 도달할 수 없다는 것.)
- *options:* **(A) 밀도 상향+굶주림 시계**(둘 다 "월드 스케일 상수가 서로 맞춰진 적 없다"는 같은 문제) · (B) 밀도만 · (C) 세력권/무리 추적(신규 메커니즘) · (D) 초기·리스폰 배치만 무리 근처로.
- **RESOLVED: (A).** 육상 6종 `animal_density` **4배**(기준=튜닝이 작동하는 포식 베드의 피식자 밀도 4.2e-4/unit²; 이 월드는 1.1e-4였다) + wolf/bear `hunger` **rate·vital_drain ÷2**(아사까지 ~1일 → ~2일). **fish 제외**(물 면적 불변이라 과밀만 됨, 이번 근거와 무관).
- **실측(6000틱):** 근접 2.0%→**66.8%**, 사냥 **0→6~9회**, 포식자 아사 30→25~47, 평균 포식자 2.12→11.65.
- **기각한 대안(측정 후):** **후각 확대** — 20→70으로 3.5배 늘려도 근접 2.01%→**2.38%**. 냄새장 자체가 그만큼 안 퍼지므로 무의미. **굶주림 시계 ÷5** — 3시드에서 사냥이 8.7→**3.3**으로 **역효과**: `Hunt = hunger×(…)`라 시계를 너무 늦추면 **사냥 동기가 죽어** 굶지 않는 방랑자가 된다. ÷2가 "탐색을 감당하되 사냥할 만큼은 배고픈" 지점. **배치만 근처로(D)** — t=0만 고치고 하루 안에 흩어진다.
- **⚠️ 미해결로 남긴 것 — P_fa4c-3의 선행 리스크.** 이 월드의 포식자는 **여전히 리스폰 항온기가 떠받친다**(평균 11.65 = 상한 12). 검증: 포식자 상한만 원래대로(2/1) 낮추면 출생으로 7~8.5를 유지하지만 사냥이 1~3으로 떨어지고, **전 종 상한을 원래대로 낮추면 생태계가 붕괴**(사냥 0, 어류 7.1). 즉 **피식자 개체군조차 아직 항온기 의존**이다. PD5(respawn→rescue floor)가 착지하려면 이걸 먼저 풀어야 한다. 가드는 이 항목을 **단언하지 않고 로그만** 남긴다(바람을 테스트에 새기지 않기 위해).
- **신설 가드:** `TestMeadowSustainsPredators` — 작은 픽스처 베드들이 **구조적으로 볼 수 없는** 것(실제 맵에서 포식자가 무리를 찾는가, 포식이 일어나는가)을 배포 월드에서 단언. 비율 목표가 아니라 "메커니즘이 실제 맵에서 발생하는가"의 바닥선.

#### PD9 — 포식자의 탐색 방식 — **RESOLVED: 발자취/체향 잔향(spoor) · 같은 채널 + 채널별 opt-in (사람 승인 2026-07-19)**
**현황 조사:** 포식자의 탐색에는 **모델이 없었다** — 냄새가 없으면 헤딩 유지 + 무작위 흔들림(상관 랜덤워크)이 전부. 더 근본적으로 **동적 층은 매 틱 재구축**되므로 세계에는 *"동물이 어딘가에 있었다"는 기록 자체가 존재하지 않았다*: 포식자는 살아있고 한두 셀 안에 있는 개체만 감지할 수 있었다. PD8이 밀도로 완화한 것의 뿌리가 이것이다.
- *options:* **(A) 잔향/spoor** · (B) 짐승길(navmap wear — 아래 참조) · (C) 행동권/세력권(개체별 신규 상태) · (D) 보류.
- **RESOLVED: (A).** 하위 결정도 사람 승인: **기존 채널에 얹되 채널별 opt-in**(별도 `scent.spoor` 채널·오퍼랜드 신설 대신) ⇒ **신규 §6 오퍼랜드 0개, 기존 늑대/곰 수식이 그대로 작동**. 신선함은 세기로 구분된다(산 개체가 가장 진하고 오래된 자취일수록 연해져, 냄새 기울기를 따르면 자연히 최근·근접 쪽으로 향함).
- **설계(정본 = `engine/space/scent/SPEC.md` §Trail):** 제3의 층. 공기 중 플룸과 두 가지가 다르고 **둘 다 물리적 근거**가 있다 — ①**확산시키지 않는다**(반복 확산=열방정식이라 자취가 균일한 안개로 뭉개져 따라갈 기울기가 소멸; 땅에 묻은 냄새는 애초에 안 날린다) ②**바람이 아니라 기울기를 따라간다**(플룸은 거슬러 올라가 발원지를 찾지만 자취는 어디로도 날아가지 않는다 — 여기에 upwind 규칙을 적용하면 발밑의 자취를 가로질러 걷게 된다). Read가 두 방향을 각자 풀어 **기여 비중으로 혼합**하므로, 생체 냄새가 지배적이면 기존과 동일하게 동작한다.
- **실측(라이브 초원 5시드, 6000틱, 사냥 횟수 평균):** OFF **7.4** → 0.006 **10.4** → **0.010 = 13.6(채택)** → 0.016 15.0. **잔향이 포식을 거의 두 배로 만든다.**
- **⚠ 단조롭지 않다:** 포화값 `strength/(1-decay)`가 `cap`을 크게 넘으면 방문 셀이 전부 상한에 붙어 **장이 평평해지고 기울기가 사라진다** — 0.030에서 사냥 21→5로 붕괴. 고원의 아래쪽 끝을 택함. 이 제약은 단위 테스트로 고정(`TestLiveScentStillOutweighsASaturatedTrail`).
...
#### P_fa4c-3 / PD5 — respawn → rescue floor — **RESOLVED: 임계=0, 완전 멸종에만 개입 (사람 승인 2026-07-20) · SHIPPED**
**차단 해제 확인이 선행.** PD8 시점에는 "전 종 상한을 낮추면 생태계 붕괴(사냥 0)"였으나, 밀도 4배·굶주림 시계·잔향이 쌓인 뒤 재측정하니 **피식자 46.0(하한 6 대비 7.7배)·포식자 5.38(하한 2 대비 2.7배)** — 항온기가 아니라 **번식이 개체군을 떠받치게 됐다.** 그래서 지을 수 있게 됐다.
- **구현:** `deficit = target - count`를 매 주기 전량 채우던 **경성 항온기**를 제거. `count > 0`이면 **아무것도 하지 않는다.** 임계=0이므로 한 마리라도 살아 있으면 개체군 동역학은 완전 창발(D2)이고 종은 실제로 폭락할 수 있다. 되돌리는 것은 **진짜 멸종뿐**(메타개체군 '이주민 도착').
- **도출된 제약 — 재도입은 창시자 *집단*이다.** 번식이 2-parent(P_fa4c-2)라 **1마리 재도입은 무의미**(같은 멸종을 반복할 뿐). `respawn_targets`를 **창시자 수**로 재해석하고 전 픽스처를 그에 맞게 재저작(피식자 4~6, 포식자 4). 발명이 아니라 짝짓기 계약에서 따라 나오는 값이다.
- **실측(라이브 초원):** 피식자 45.9 · 포식자 6.49 · 포식 3 · 근접 53.6%. 이전 대비 개체수는 절반이지만 **항온기 없이 유지된다.**
- **⚠ 항온기가 가리고 있던 것들이 드러났다(이번에도):** ①**어류가 최약체**(평균 3.6, 창시자 4 근처 — 정주성이라 고갈 패치를 못 벗어남). 가드를 "하한 초과"에서 **"멸종하지 않는다"**로 정정(하한이 더 이상 유지 수준이 아니므로 옛 단언은 무의미). ②`TestMinimalMeadowRandomFixture`의 **anti-east-march 가드가 top-up에 전적으로 의존**하고 있었다(리스폰이 만든 개체들만 헤딩이 다양했다) ⇒ 여전히 유효한 곳(밀도 배치)으로 이전(`TestDensityHeadingsVaried`). ③여러 베드가 항온기 위에 서 있었음 — 전부 새 의미로 재작성.
- **PD10 콘텐츠 예시 철회:** 염소 `terrain.slope` 선호를 **되돌렸다.** 개체수가 절반이 되며 신호가 잡음에 묻혔고, 계수를 키우면(1.20) **`move_deadband` 아래로 떨어져 어디서나 멈춰** 선택 자체가 사라진다(0.60·1.20 모두 0.100으로 동일). **오퍼랜드(메커니즘)는 SHIPPED·단위 테스트 유지**, 저작 예시만 철회 — 작동하지 않는 예시를 콘텐츠에 남기면 거짓 주석이다. 종별 서식지 저작 = 별도 튜닝 라운드.

#### PD11 — §7 노화(senescence) — **RESOLVED: (i-a) 오퍼랜드 · (ii-b) 노쇠-vital 출혈 · (iii-b) 압축 스케일 (사람 승인 2026-07-20)**
사람 승인: "노화 + 컨디셔닝(쓰면 늚) 함께"(2026-07-20) → 사람 지시로 **노화부터** 착수. 컨디셔닝(D7 사용-기반 상승)은
fauna를 넘어 agent 스탯까지 가므로 **별건**으로 남긴다. 여기서 결정할 것은 노화 축의 하위 메커니즘뿐이다.

**현황 실측(코드 기준, 2026-07-20):**
- `Animal.Age`는 매 틱 `+FaunaDT` 누적되지만 **하는 일이 `maturity = clamp01(Age/maturity_age)` 하나**뿐 — 0→1로 오른 뒤
  영원히 1. **전성기도 노쇠도 없다.**
- 사망 원인은 `causePredation`·`causeStarvation` **둘뿐**(`world.go:72`) ⇒ **수명사가 없어 동물은 생물학적으로 불사**다
  (포식·기아를 면하는 한 무한히 산다).
- **§6 DSL로는 곡선을 저작할 수 없다**(`expr/SPEC.md`): min/max/clamp 함수 없음, 삼항 없음, bool→num 승격 없음,
  **단항 마이너스조차 없다**(`0 - x`로 써야 함). ⇒ 곡선은 **엔진이 계산해 오퍼랜드로 노출**하는 수밖에 없다 —
  `maturity`가 이미 그 패턴이다(Go에서 clamp01, 종별 파라미터는 content).
- 재사용 가능한 사망 기계가 **이미 있다**: `DriveRule.VitalDrain`/`VitalDrainAbove`(θ, r) — PD3 기아사가 쓰는
  drive-포화→vital 출혈 결합. 주석이 "thermal-freeze later reuses the same path"라고 이미 예고해 둠.

- **PD11-i — 노쇠를 무엇으로 표현하나.** *options:*
  **(a) 신규 §6 오퍼랜드 `senescence = clamp01((age − prime_age)/(lifespan − prime_age))`** — `maturity`와 정확히
  대칭(상승지/하강지). 저장 상태 0개, 직렬화·골든 무교란, 미저작 종은 0(off-lever). 콘텐츠가 `speed`/`hit`/`attack_power`
  수식에서 감산해 **무엇이 늙는지를 데이터가 정한다**(D10).
  **(b) 기본 스탯 `a.Stats`를 매 틱 실제로 감쇠** — D7 문자 그대로. **그러나 PD4-iv 유전과 충돌한다**: offspring 스탯 =
  부모 평균 ± 변이이므로, 늙어 약해진 부모가 **약한 자식을 낳는 라마르크식 유전**이 된다(획득형질 유전). 직렬화·골든도 변함.
  **(c) 단일 `vigor` 종형 곡선**(maturity 흡수) — 오퍼랜드 1개지만 두 국면이 한 스칼라에 묻히고 기존 maturity 게이트와 중복.
  **rec: (a)** — 유전 오염이 없고, DSL 제약상 어차피 엔진 계산이며, 기존 `maturity` 패턴의 거울.
  ⚠️ (a)를 고르면 D7의 "기본 능력치는 나이로 변동"은 **표현(expressed) 층에서** 구현된다 — 유전자(저장 스탯)와
  표현형(=저장 스탯 ⊗ 나이 계수)의 분리. 이 구분은 **컨디셔닝 축에도 그대로 필요**하므로 미리 정해두는 값이 있다.

- **PD11-ii — 수명(사망 채널).** *options:*
  **(a) 수명사 없음(간접)** — 노쇠가 speed/hit만 떨어뜨려 포식·기아로 **간접** 사망(D2 창발, 새 사망 원인 0).
  ⚠️ 그러나 이 세계에서 **잘 안 움직이는 개체는 사실상 불사로 남는다**(정주성 어류, 포만한 피식자는 FM14b상 애초에 안 움직임).
  **(b) 노쇠-vital 결합 — 기존 PD3 θ/r 재사용** — `senescence ≥ θ`면 vital이 서서히 출혈 → **부드러운 노쇠사**.
  신규 기계 최소(기존 필드 패턴 복제). **새 사망 원인 라벨 `causeSenescence` 추가**(AnimalDied telemetry에서 기아와 분리 —
  안 나누면 노화 도입 후 "기아 급증"으로 오독된다). ⇒ 이벤트 `cause` 값이 하나 늘어난다(데이터 계약 변화).
  **(c) `max_age` 하드컷 즉사** — 가장 단순하나 계단 함수라 코호트가 동시에 죽고, 노쇠 곡선과 이중 저작.
  **rec: (b)** — (a)는 관측 가능한 효과가 거의 없고, (c)는 (a)의 곡선과 따로 놀아 저작이 갈라진다.

- **PD11-iii — 생애 시간 스케일 (⚠ 이게 제일 큰 쟁점).** 현 `maturity_age` 값은 **250~1000틱 = 4~17 게임시간**이다
  (1틱=1게임분, 1일=1440틱, 1년=172,800틱). 즉 **사슴이 10시간 만에 성체가 된다** — 기존 값은 전부
  `UNTUNED placeholder`라고 주석돼 있고, 달력과 맞춰본 적이 없다. 수명 값은 이 스케일 위에서 정해진다. *options:*
  **(a) 현실 달력** — 사슴 ~10년 = 1.73M틱. 생물학적으로 정직하지만 **관측 런(6,000틱)에서 노화는 절대 관측되지 않는다**
  ⇒ 이번 라운드에 저작해도 **전 가드가 off-lever와 구별 불가**(작동 여부를 확인할 수 없는 콘텐츠 = PD10에서 철회한 그것).
  **(b) 기존 압축 스케일 유지, 수명 = `maturity_age × k`** (k≈8~15 ⇒ 사슴 6,000틱 ≈ 4게임일) — 지금 돌리는 베드에서
  **세대교체가 실제로 보인다**. 대가: "나흘 살고 늙어 죽는 사슴". 어차피 성숙도 이미 그 스케일이라 **내부 정합은 유지**된다.
  **(c) 생애 시간 전체를 달력으로 재기준**(maturity_age까지 재저작) + 관측용 별도 `time_scale` 노브 — 정직하지만
  **번식·성숙 밸런스를 방금 안정화한 개체군 위에서 전부 다시 맞춰야 한다**(PD8/PD5 재교정 반복).
  **rec: (b)** — 이번 라운드에서 **작동을 실측할 수 있는 유일한 선택**. (c)는 할 만한 별도 라운드로 남긴다.

- **확정 설계(위 셋의 합):** `senescence = clamp01((age − prime_age)/(lifespan − prime_age))` 오퍼랜드 +
  `senescence ≥ θ`일 때 `vital −r/tick` + 신규 사망 라벨 `causeSenescence` + 수명 = `maturity_age × k`.
  구현 정본 = `backend/engine/fauna/SPEC.md` §Senescence / `SPEC-combat.md` §Vital.

- **1차 실측(라이브 초원 6000틱, 노화 ON) — 작동하지만 코호트가 동시에 죽는다.**
  사망 원인: 기아 80 · **노쇠 54** · 포식 5. 노쇠사 종별: 염소 16 · 토끼 17 · 사슴 12 · 물고기 9(늑대·곰은 수명 7.2k/12k라 이번 런에선 안 늙음).
  **메커니즘은 확실히 작동한다** — 이전엔 이 세계 동물이 포식·기아를 면하면 생물학적으로 불사였다. 그런데 개체수 궤적이:

  | t | 1000 | 2000 | 3000 | 4000 | 5000 | 6000 |
  |---|---|---|---|---|---|---|
  | 합계 | 58 | 56 | 43 | 45 | 23 | **17** |
  | 토끼(수명 2400) | 19 | 18 | **6** | 6 | 2 | 6 |
  | 염소(수명 5000) | 16 | 17 | 17 | 18 | **3** | 3 |
  | 사슴(수명 6000) | 12 | 12 | 12 | 12 | 13 | **1** |
  | 늑대 | 3 | 3 | 1 | 1 | 1 | **0** |

  **각 종이 정확히 자기 수명에서 무너진다.** 원인은 밸런스가 아니라 **초기 배치**다: worldgen이 배치하는 모든 동물이
  `Age = 0`(`worldgen.go:281` — Age 필드를 아예 세팅하지 않는다)이라 **창시 개체군 전체가 단일 코호트**이고, 수명에
  도달하는 순간 함께 죽는다. 늑대 절멸(0)은 그 2차 효과다 — t=5000에 염소·토끼가 동시에 사라지자 포식자가 굶었다.
  대조: 노화 이전 같은 월드는 피식자 45.9 · 포식자 6.49 · 근접 53.6%였고, 지금은 38.6 · 5.85 · 37.6%다.
  ⇒ **노화 자체의 밸런스를 논하기 전에 연령 구조가 먼저다.** 지금 숫자는 노화의 효과가 아니라 코호트 인공물이 지배한다.

- **PD11-iv — 창시 개체군의 연령 구조 — RESOLVED: (a) 배치 시 `Age ~ U(0, lifespan)` (사람 승인 2026-07-20) · SHIPPED.** 수명이 생긴 이상 "모두 신생아로 시작"은
  성립하지 않는다. *options:* **(a) 배치 시 `Age ~ U(0, lifespan)`** — 코호트를 즉시 깬다. 배치당 RNG draw 1개(D12
  draw-count 계약 주의). 창시자의 ~25%가 이미 노쇠 구간이라 t=0 직후 작은 사망 파도가 인다. **(b) `U(0, prime_age)`** —
  즉시 사망은 없지만 나이가 앞쪽 45%에 몰려 **더 늦게 같은 파도**가 온다(문제를 미루는 것에 가깝다). **(c) 생존곡선 가중
  분포**(어릴수록 많음) — 가장 현실적이나 곡선을 새로 정해야 한다. **(d) 방치** — 장기 런에서 씻겨나간다고 보고 인공물을
  감수. **rec: (a)** — 기계가 최소(draw 1개)이고, 우리 사망 모델(수명 말기에 집중된 노쇠사 = type-I 생존곡선)에서는
  균등분포가 곧 정상 연령분포에 가깝다. rescue floor의 창시자 집단에도 같은 규칙을 적용(이주민도 성체 무리다).
  - **구현:** `AnimalPlacement.Age`(fixture `age:` 키, 기본 0) + density 배치가 draw. **명시 배치는 손대지 않는다** —
    저작자가 id·위치를 적어 넣은 개체는 나이까지 저작자의 말이고, 생성된 개체군만 생성 규칙을 따른다. rescue 창시자도 동일.
  - **⚠ 배운 것 — 나이 draw를 기존 스트림에 얹었다가 지도를 통째로 재굴림했다.** 첫 구현은 density의 공용 `r`와
    respawn의 `fork`에서 뽑았는데, draw 하나가 **그 뒤의 모든 위치·헤딩을 이동**시켜 노화와 무관한 월드까지 바뀌었다
    (포식 아레나의 피식자가 일시 절멸 — 아레나는 수명 9600 > 3000틱이라 **노화가 일어나지도 않는 베드**였는데도).
    ⇒ 나이는 **전용 스트림**(worldgen `ageSeedSalt`, world `envFork(tick,"respawn_age")`)에서 뽑는다. 기존 배치는
    바이트 동일하게 남고 변경이 한 가지로 고립된다. 격리 후 재측정: 피식자 37.7→**44.0**, 근접 26%→**37.6%**
    (즉 앞선 악화의 상당 부분은 노화가 아니라 스트림 이동이었다).

- **k 재적합 (2026-07-20) — 처음 잡은 k≈10이 생태계를 무너뜨렸다.** 사슴 6000틱 수명에서 라이브 초원은 6000틱 동안
  58→17로 반토막 나고 **포식 0회**. 원인은 노화율이 아니라 **수명이 이 지도에서 짝을 만나는 데 걸리는 시간보다 짧다**는
  것 — 이동 ~0.2 units/tick × 6000틱 = 평생 1,200 units(500² 지도 2회 횡단)라, 개체군이 한 번 낮아지면 번식으로 회복하지
  못하는 **Allee 구간**에 갇힌다. **20,000틱 장기 관측이 결정적이었다: 곰(최장수 + 잡식)만 4→57로 폭증**하고 나머지는
  전멸 직전 — 수명 차이가 곧 경쟁 우위가 된다. **k≈32~48로 상향**(사슴 24k·늑대 28.8k·염소 20k·토끼 9.6k·물고기 8k·곰 48k).
  **관측 가능성은 잃지 않는다**: PD11-iv로 창시자 나이가 흩뿌려지므로 수명이 길어도 노쇠사는 t=0부터 난다 — 이 두 결정은
  서로를 가능하게 한다. (PD11-iii에서 (b)를 고른 이유였던 "6000틱 런에서 관측 가능"이 k 상향 후에도 유지되는 근거.)
- **속도 저하는 원인이 아니었다(귀속 실험).** `- senescence * X`를 전 종에서 제거해도 개체수 평균 31.2→30.6으로 사실상
  동일(기아 123→97 · 노쇠 53→69로 **라벨만 이동**). 개체군을 움직인 것은 vital 출혈, 즉 사망률 자체다. ⇒ 노브를 만질 곳은
  능력 저하 계수가 아니라 수명이었다.
- **최종 A/B (라이브 초원 6000틱, 같은 시드):**

  | | 노화 OFF(lifespan=0) | 노화 ON |
  |---|---|---|
  | 개체수 평균 / 최소 / 최종 | 58.7 / 52 / 55 | **50.9 / 40 / 40** |
  | 사망: 기아 · 포식 · 노쇠 | 83 · 3 · — | 72 · 2 · **20** |
  | 피식자 평균 / 포식자 평균 | (노화 전 기록 45.9 / 6.49) | **44.0 / 4.83** |

  노쇠사가 6종 **전부**에서 발생하고(토끼 8 · 사슴 3 · 염소 3 · 늑대 3 · 물고기 2 · 곰 1), 개체군은 유지된다.
  비용은 개체수 −13%. **어류는 여전히 최약체**(평균 3.9→2.0) — 정주성이라 모든 추가 사망률이 어류를 먼저 친다(PD5와 동일 패턴).
- **신설 가드 `TestAgingHappensOnTheLiveWorld`** — ①노쇠사가 실제로 발생 ②세계가 비지 않음 ③**창시 개체군이 t=0에
  연령 구조를 가짐**. ③은 **종료 시점 나이 분산으로는 못 잡는다**(코호트 런도 5401틱 분산을 보였다 — 출생이 흩뿌리므로).
  뮤테이션 2종으로 검증: 전 종 `lifespan: 0` ⇒ ①이 FAIL, density `Age: 0` ⇒ ③이 6종 전부 FAIL.
- **부수 수정 — 사망 원인 분류가 `!= "starvation"`이었다.** 세 번째 원인이 생기자 **노쇠사가 포식으로 집계**되어
  `TestMeadowSustainsPredators`가 포식 0인 세계에서도 통과할 뻔했다. 4개 베드를 전부 **정확 일치**로 교정.

- **선행 경고(반복하지 않기 위해 미리 적음):** 노화는 **새 사망 채널**이다. 방금 PD5에서 "항온기 없이 자립한다"를 확인한
  개체군에 사망 요인을 얹으면 두 변경의 효과가 뒤섞인다 ⇒ **노화는 자체 before/after 실측과 함께 단독으로 착지**시키고,
  기존 가드(`TestPredatorBalanceAcrossSeeds`·`TestMeadowSustainsPredators`·`TestForageReachability`)를 노화 ON/OFF로
  각각 돌려 귀속한다. 그리고 PD10의 교훈대로 **실측되지 않는 저작은 남기지 않는다.**
...
- **신설 가드(재교정):** ①**영양 서열 트립와이어** — 포식자가 피식자보다 **크게** 더 굶주리면(>0.15) 실패. 비율 목표가 아니라 **구조적 단언**이고, 재교정 이전 상태(0.657 vs 0.355)를 정확히 잡는다(뮤테이션 검증). 기존 개수 단언들은 이 병증을 **하나도** 못 잡았다(양측이 형식상 공존했으므로). ②**다시드 가드** `TestPredatorBalanceAcrossSeeds` — 단일 시드 편차가 크므로(같은 밸런스에서 포식 1~26) 한 시드에 맞춰 튜닝하는 과적합을 차단.
...
- **PD4 — 창발 번식 (emergent birth — respawn top-up 대체).** *options:* (a) **무성 근접 spawn** 포만+성숙(vital↑) 개체가 slow cadence로 근처에 offspring(drive-gated roll) — **flora 씨앗분산 모델(§1a) 그대로 재사용**(부모근접·seeded·carrying_capacity 가중). (b) 2-parent 짝짓기(신규 mate-seeking action 필요). (c) 에너지예산(PD3c와 결합, 출산이 reserve 소모). rec였던 것은 (a)이나 **RESOLVED: (b) 2-parent 짝짓기 (사람 승인 2026-07-18).** — 더 풍부한 사회적 창발(짝 탐색·만남). **PD4b(carrying capacity):** flora `carrying_capacity` §6 **재사용**(동종밀도 logistic로 출산 억제 — 하드캡 없이 폭주 방지, 이미 RESOLVED). **PD4c(cadence/결정성):** flora와 동일(world per-step fork, `tick%N` bulk, D12).
  - **⚠️ (b)는 그린필드 — `repro_readiness`(F19/F25)는 어휘만 있고 미배선, age/성숙 필드 없음.** 하위 게이트(사람 승인 2026-07-18):
    - **PD4-i 짝 탐색: RESOLVED (i) 근접 spatial-hash 질의.** combatTarget 기계 재사용 — 근거리 동종 성숙+준비 최근접 파트너를 스페이셜 해시로 질의. 신규 scent 채널/deposit 없음(성능·결정성 안전). 짝짓기는 근접이므로 적합.
    - **PD4-ii 준비/성숙 게이트: RESOLVED (ii) 신규 `Age` 필드.** 신규 직렬화 상태필드(per-tick +DT). 성숙 = `age ≥ maturity_age`(종별 content). ⚠️ **직렬화/골든 변화**(Animal 스냅샷) + **§7 lifecycle과 연결**(age→노화는 후행 별개 트랙, 이번엔 성숙 게이트만). 쿨다운은 age가 아니라 **별도 `MateCooldownUntil` 틱**(EngageCooldownUntil 패턴 재사용, SPEC 상세).
    - **PD4-iii 수정→offspring: RESOLVED (i) 즉시 근접 offspring.** 두 성숙+준비 파트너가 근접(exchange 사거리 유사)하면 seeded 수정 roll → 즉시 부모 근처 offspring 1(저-ID 부모가 spawn, 양쪽 MateCooldown 설정), flora 번식 cadence(tick%N bulk fork) 재사용, D12.
    - **PD4-iv offspring 스탯 — OPEN (P_fa4c-2 차단; 2026-07-19 재확인: 여태 `rec`였고 RESOLVED로 뒤집힌 적 없음).** offspring을 spawn하려면 스탯을 반드시 정해야 하므로 c-2의 **경성 선행**이다. **코드 실측:** `engine/mind/stats`에 이미 per-stat **`Inherit float64` ∈[0,1]**(stats.yaml 저작, `stats.go:76/107`)가 존재 — "재사용"은 실체가 있는 말. 현 respawn은 종별 **템플릿 Animal**(`InstallRespawn(templates …)`)을 복제할 뿐 유전이 없다. *options:* **(a) 부모 평균 + 변이, 기존 per-stat `inherit` 가중치 재사용** — 혈통·선택압이 창발(D2: "빠른 사슴"이 데이터 없이 등장); 출생당 RNG draw 추가(D12 draw-count 계약 주의). **(b) 종 템플릿 그대로**(현 respawn과 동일) — 신규 메커니즘 0·가장 안전하나 **유전이 없어 진화적 창발 불가**(번식이 개체수 숫자놀음으로 축소). **(c) 부모 평균만(변이 없음)** — 중간이나 세대마다 분산이 붕괴해 종이 균질해짐. **rec: (a)** — 기계가 이미 있고, 2-parent를 (b) 대신 고른 이유("더 풍부한 사회 창발")가 유전 없이는 회수되지 않음. **RESOLVED: (a) 부모 혼합 + 변이 (사람 승인 2026-07-19).** offspring stat = 부모 평균 ± 변이, 변이 폭은 `stats.yaml`의 **기존 per-stat `inherit` 가중치**(`stats.Registry`, D7 정합) 재사용. `Age=0` 출생. **D12:** 출생당 draw 수를 고정하고 world per-step fork에서 뽑는다(스탯 id 정렬 순회).
    - **PD4-v 출산 밀도 게이트 (PD4b의 미열거 하위) — OPEN.** PD4b는 "flora `carrying_capacity` §6 재사용"으로 개념 RESOLVED이나, **코드 실측 결과 fauna에는 재사용할 기계가 없다:** `carrying_capacity`는 flora 블록 전용(스키마 설명도 flora 전용)이고, fauna `AttrOperands()`에 **동종 밀도/이웃 수 operand가 없다**(있는 것은 scent.*/dist.*/maturity/…). 단 `Snapshot.Spatial`+`ByID`는 있으므로 **계산은 가능**하다. 즉 "재사용"이 아니라 **신규 operand+키**다. *options:* **(a) c-2에 함께** — 신규 §6 operand(`kin_density` 류, spatial 질의로 반경 내 동종 수)+종 블록 `birth_capacity` K → 수정 roll에 `max(0,1−n/K)` 곱(flora 1k와 동형). **(b) utility에만** — operand만 추가하고 종 §6 Mate utility에서 `- kin_density*w`로 억제(신규 키 0, 부드럽지만 상한 보장 약함). **(c) c-3으로 이연** — c-2는 짝짓기만, 상한은 P_fa4b 아사 + respawn 하네스가 임시 담당, c-3에서 rescue floor와 함께 착지. **rec: (c)** — §5의 자기 안전계약("매 phase는 **새 메커니즘 하나만** respawn 하네스에 대고 관측")과 정합하고, (a)는 c-2에 메커니즘 2개(짝짓기+밀도장)를 동시에 얹는다. ⚠️ 단 (c)는 관측 기간 동안 개체수 폭주 위험을 아사에만 의존하므로, c-2 검증에서 **개체수 상한 tripwire**를 반드시 볼 것. **RESOLVED: (c) c-3으로 이연 (사람 승인 2026-07-19).** c-2는 짝짓기만 얹고, 상한은 P_fa4b 아사 + respawn 하네스가 임시 담당. **c-2 수용 조건에 개체수 폭주 tripwire 필수**(장기 런에서 종별 상한 확인).
    - **PD4-v 방식 확정 — RESOLVED (A 먹이 희소성 + C-약 연성 밀도항) (사람 승인 2026-07-19).** **결정을 뒤집은 측정(12000틱):** 토끼 15→264로 느는 동안 **식물이 2125→38028로 더 빨리 늘었고 평균 허기는 0.16~0.33** — 즉 초식동물은 내내 배불렀고, **모든 종 Mate 효용에 이미 들어있는 `(1 - hunger)` 브레이크가 걸릴 일이 없었다.** ⇒ 폭주의 원인은 "밀도 제한 부재"가 아니라 **희소성·포식의 동시 부재**이며, 인공 상한을 얹으면 그 두 결함을 덮는다. **현실 근거:** 밀도 조절은 개체군 회계가 아니라 *밀도가 오를수록 개체에게 나빠지는 일들*의 창발이고(D2 동형), 큰 포유류에서 무너지는 순서는 ①새끼 생존 ②첫 출산 연령 ③성체 출산율 ④**성체 아사(맨 마지막·붕괴 시)** — 즉 조절은 압도적으로 **출산 쪽·몸상태 매개**다. 우리 심엔 ④만 있었다. (포식자·먹이 둘 다 없으면 현실에서도 폭증 후 대붕괴한다 — 세인트매튜섬 순록·카이밥 사슴. 지금 상태가 정확히 그것.)
      - **주 레버 = 튜닝(신규 메커니즘 0):** `graze_depletion_per_hunger`↑ · flora `length_rate`↓ · herbivore hunger `rate`↑ ⇒ 기존 `maturity*(1-hunger)` 사슬이 비로소 물린다(= "공유지의 비극" 시나리오 발화).
      - **안전망 = 연성 밀도항(신규 operand 1개, 경성 상한 아님):** 종 Mate 효용에 `- kin_count * w`. 세력권/간섭 경쟁의 서술을 유지하면서 연속·창발적으로 억제. **경성 `max(0,1−n/K)` 게이트는 채택하지 않음**(이웃 수를 세어 K로 나누는 것은 개체군 회계를 개체 수준에 심는 것 = D2에서 가장 멂).
      - **⚠️ 이름 변경(구현 중 판단):** 승인 프리뷰의 `kin_density`는 실제로는 **반경 내 동종 마릿수(정수)** 라, 정규화 상수를 새로 발명하지 않는 한 "density"가 아니다. 그래서 operand 이름을 **`kin_count`** 로 착지하고 정규화는 content 수식에 맡긴다(`- kin_count * 0.03` 식). `dist.*`/`scent.*`가 이미 원시 크기를 노출하는 방식과 동일(D10).
      - **(B) 포식 복구는 별도 트랙**(늑대·곰이 사냥 전에 굶어 죽는 문제) — c-3이 아니라 후속 튜닝 단계.
      - **구현·측정 결과 (2026-07-19, rabbit_meadow 500² / 14000틱):** 착지값 = `graze_depletion_per_hunger` 0.5→**2.0** · deer hunger rate 0.0008→**0.0014** · rabbit 0.0012→**0.0020** · flora `carrying_capacity` 계수 grass 12→**5**/tall_grass 10→**4**/wildflower 6→**3** · Mate 효용에 `- kin_count * w`(deer/goat 0.04, rabbit 0.10). **결과: 토끼 15→30(튜닝 전 동일 조건 2407 = 약 80배 감소) · 사슴 8→8~11 안정 · 염소 5→6 안정 · 식물 2125→11936에서 평형 도달**(증가율 붕괴: 10673→11473→11936). **가장 큰 단일 레버는 flora carrying_capacity**였다 — 초원 총량이 천장을 정한다. **잔여 2건:** ①토끼는 아직 완만한 상승 추세(30@14k)라 상한이 *증명*되진 않음 — 더 긴 런 필요 ②**늑대·곰이 사실상 멸종**(측정 구간 대부분 0) = (B) 포식 트랙의 문제이지 밀도 트랙 문제가 아님.
      - **⚠️ 측정 함정(기록):** `go test`는 **콘텐츠 YAML 변경으로 캐시를 무효화하지 않는다** — 튜닝 후 측정이 이전 실행과 바이트 동일하게 나와 "변경이 무효"로 오독할 뻔했다. 콘텐츠만 바꾼 뒤 재측정할 때는 **반드시 `-count=1`**.
    - **PD4-vi 수정 파라미터 3종 — OPEN(형태 결정, 값 아님).** ⓐ **수정 사거리**: exchange 사거리 재사용 vs 종별 키. ⓑ **`MateCooldownUntil` 길이**: 전역 고정 vs 종별 vs "임신기간" 개념 도입. ⓒ **상호 선택**: 양쪽 모두 Mate를 골라야 성립(합의) vs 한쪽이 접근하면 성립. *rec:* ⓐ exchange 재사용 · ⓑ 종별(`mate_cooldown`, 번식 속도가 종 특성) · ⓒ **양쪽 합의** — 한쪽 성립은 §6 utility를 우회해 "싫어하는 개체도 번식"이 되어 D1/D3 정신에 어긋남. **RESOLVED: 추천 3개 일괄 (사람 승인 2026-07-19).** ⓐ 수정 사거리 = 기존 exchange 사거리 재사용(신규 키 0) · ⓑ `mate_cooldown` 종별 스칼라(≤0/미저작 ⇒ 쿨다운 없음) · ⓒ **상호 합의** = 양쪽 모두 그 틱에 `Mate`를 선택했을 때만 수정 성립(§6 utility가 우회되지 않음 ⇒ 배고픔·공포가 번식을 실제로 억제).
...
- **PD5b — 물고기: 감소만 가능한 개체군 (P_fa4c-3 선행) — OPEN (2026-07-19 제기).** **문제:** fish는 `hunger` drive가 **없고**(diet·Graze·graze 수식·Mate 전부 없음) `game` 태그로 **잡아먹히기만** 한다 ⇒ **증가 경로가 0인 개체군**. 지금은 `respawn_target: 20` 항온기가 채워 가려져 있지만, **PD5(respawn→rescue floor 강등)가 착지하는 순간 물고기는 확정 멸종 경로**에 들어간다 ⇒ **c-3의 선행**. 현 상태는 content 주석에 "수생 먹이는 수생 flora 필요 ⇒ DEFERRED"로 명시된 **의도된 미구현**이나, c-3가 그 대가를 청구한다.
  - **실측(2026-07-19): 벽이 낮다.** `depth`는 이미 실 지형 속성(river 0.5 / lake 0.7 / sea 1.0 / 육지 0.0)이고, **flora를 물에서 배제하는 코드는 없다** — 모든 flora 종이 `carrying_capacity`에 `(1-depth)`를 곱해 스스로 빠져 있을 뿐이다. 즉 수생 flora = **부호를 뒤집은 §6 수식 한 벌**(순수 content, D10, 엔진 변경 0). ⚠ 단 pos-less 배치(`materialize.go`)는 soil 헥스만 고르므로 수생 종은 **명시 pos 또는 density 배치**(density는 carrying_capacity를 따르므로 물만 고름)여야 하고, density는 `terrain: random:true` 픽스처에서만 동작(rabbit_meadow는 해당 ✓).
  - *options:* **(A) 수생 flora + 물고기 루프 완성** — 신규 flora 종(예 `waterweed`: `suitability`/`carrying_capacity`가 depth·moisture에 비례, `scent:food`+`forage` 태그) + fish에 `hunger` drive·`Graze` 액션·`graze` 수식·`diet:[forage]`·`Mate`/`maturity_age`. 물고기가 먹고·굶고·번식하고·먹히는 **완전한 참여자**가 된다. 비용=콘텐츠 한 종+fish 블록 확장. **(B) 번식만 부여**(Mate만, 먹이 없음) — 최소 변경이나 **먹이 상한이 없는 개체군**이라 포식·`kin_count`만이 제동. 초식동물에 대해 방금 확립한 "공짜 점심 금지"와 정면 충돌. **(C) 물고기만 rescue floor에서 제외**(top-up 유지, '산란 회귀' 추상) — 신규 콘텐츠 0이나 창발 세계에 항온기를 특례로 남긴다. **(D) 수생 flora 착지 전까지 물고기를 월드에서 제외** — 반쪽 생물을 없애지만 F18 수생 통행 시연(물고기가 유일한 사례)을 잃는다.
  - **RESOLVED: (A) 수생 flora + 물고기 루프 (사람 승인 2026-07-19) → 부분 SHIPPED.** 신규 flora `waterweed`(`suitability: depth*0.9+moisture*0.1`, `carrying_capacity: 10*depth*moisture` ⇒ 육지 0·강 5·바다 10, `scent:food`+`forage`) + fish에 `hunger` drive(vital_drain 포함)·`Graze`·`graze`·`diet:[forage]`·`Mate`·`maturity_age`/`mate_cooldown`·`scent_acuity` 저작. **엔진 변경 0(전부 content).** 실측: 수초 2500→3262(물에만 정착·번식 확인 — 육지면 suitability 0.05<θ로 고사), 물고기 Graze 96.4%, 개체수 유지. **✅ 완결 (2026-07-19, 2차) — 원인은 콘텐츠가 아니라 엔진 결함 2건이었고, 둘 다 전 초식종에 걸쳐 있었다.** `nearestForageFloraID`는 빈손이 아니었다 — **먹을 게 없는 식물을 계속 돌려주고 있었다.**
    - **결함 1 — 고갈된 식물이 계속 "가장 가까운 먹이"로 선택됨.** 동물이 옆 식물을 Length 0까지 뜯으면 그 식물은 **영원히 최근접 emitter로 남고**, `GrazeLength`는 남은 생물량만큼만 주므로 이후 모든 Graze가 조용히 0을 회수한다. 실측(라이브 초원): 최근접 먹이식물이 고갈된 개체인 비율이 **rabbit 95% · deer 97% · goat 96% · fish 99%**, 그리고 그 중 **약 80%는 같은 사거리 안에 생물량 있는 식물이 있었다** — 즉 **먹이 위에 서서 굶고 있었다.** 물고기는 거의 움직이지 않아 이 함정에서 못 벗어났을 뿐, **어류 고유 문제가 아니었다.** 픽스=고갈 식물 skip. 회귀 `TestGrazePassesOverExhaustedPlant`.
    - **결함 2 — 식물 냄새가 생물량과 무관한 상수였다.** flora는 `floraState`와 `w.objects` **양쪽**에 등록되는데(env.go PlaceObject), 개체별 침전이 flora를 걸러내지 않아 생물량 기반 `Length+Width` 위에 **평평한 1.0이 덧씌워지고** 있었다. 그 결과 SPEC이 명시한 PD2 되먹임("과다 채식 ⇒ 냄새 약화 ⇒ 이주 압력")이 **조용히 삭제**돼 있었다 — 동물은 고갈된 패치를 떠날 신호를 받지 못했다. 픽스=개체 침전이 `floraState` 소속을 skip. 회귀 `TestFloraScentTracksBiomass`.
    - **효과(실측):** 물고기 개체수 **6→12**(리스폰 하한의 2배 ⇒ 번식이 실제로 개체군을 지탱), 평균 허기 0.624→0.301. 포식 베드에서 **피식자 아사 대폭 감소**(rabbit 75→43, deer 28→20). ⇒ **c-3 차단 해제.**
    - **⚠️ 밸런스 재교정 필요(별도 phase).** 두 픽스는 급식 루프가 **고장 난 상태에서 교정된** 모든 수치를 무효화한다. 기여 분리(포식 베드, 6000틱): 픽스 전 = 교전 81·성공 4.9%·포식자 잔존 7 / **결함1만** 수정 = 교전 61·성공 8.2%·잔존 6 / **둘 다** = 교전 31·성공 3.2%·잔존 4. 즉 **결함1은 피식자 아사를 고치고 사냥성공률을 올리며, 결함2(냄새장 약화)는 피식자를 분산시켜 교전 빈도를 절반으로 줄인다.** 공존 단언은 계속 통과. `scent_acuity`·graze 계수는 새 냄새장 크기 기준으로 다시 잡아야 한다.
  - **원래 rec 근거:** DEFERRED의 근거였던 "수생 flora가 없다"가 **엔진 제약이 아니라 미저작**임이 확인됐고, 부호 뒤집기 한 벌이면 된다. (B)는 방금 PD4-v에서 정한 원칙(희소성이 상한을 만든다)을 물고기만 면제하는 것이라 일관성이 깨진다.
...
> **게이트 상태(개체군 동역학):** PD1=**RESOLVED (c) confidence-blend**·PD1a=**RESOLVED (b) 소비자(D12 강제)**·PD1b=**RESOLVED (c) §6 스탯 합성(`scent_acuity`)**; PD2=**RESOLVED (b) flora 고갈**; PD3=**RESOLVED (a) 유예+출혈 (사람 승인 2026-07-17)**; PD4=**RESOLVED (b) 2-parent 짝짓기 (사람 승인 2026-07-18)** — 하위: PD4-i/ii/iii **RESOLVED**(2026-07-18) → **P_fa4c-1 SHIPPED**; **PD4-iv=RESOLVED (a) 부모 혼합+변이 · PD4-v=RESOLVED (c) c-3 이연 · PD4-vi=RESOLVED 추천 일괄**(전부 사람 승인 2026-07-19; 이들이 2026-07-19에 **신규 열거**된 게이트다 — iv는 여태 `rec`였고 RESOLVE된 적 없었으며, v는 PD4b가 "flora carrying_capacity 재사용"이라 했으나 fauna에 해당 operand·키가 **없음**을 코드로 확인) **⇒ P_fa4c-2 착수 가능 ✓**; PD5=**RESOLVED (b) rescue floor**(단 하위 값/배치 OPEN ⇒ P_fa4c-3 차단); PD6=**rec PD1 우선**; PD7=**OPEN(P_fa4d, 선택)**. **P_fa4a·P_fa4b 게이트 전부 RESOLVED ✓ → P_fa4a·P_fa4b SHIPPED(아래 Phases).** 다음 빌드=P_fa4c(PD4-i/ii/iii RESOLVE 후). **핵심 안전 계약: 매 phase는 새 메커니즘 하나만 respawn 하네스에 대고 관측 — "번식 없는 아사"·"은신처 없는 창발"을 구조적으로 회피.**
>
> **PD3 RESOLVE 상세 (a) 유예+출혈 — 착지 형태 (SPEC = fauna/SPEC.md · world/SPEC-world-fauna.md):** "유예"는 **별도 카운터 필드 없이** hunger 어큐뮬레이터의 상승 시간으로 emergent(hunger는 [0,1] clamp이라 포화까지 오르는 데 걸리는 시간이 곧 유예) + vital 풀이 rate로 소진되는 시간(2차 버퍼). 기전 = **drive-포화→vital 출혈**을 `DriveRule`에 **per-drive 데이터 필드**로(`VitalDrain` r·`VitalDrainAbove` θ, D4/D10) — hunger drive가 θ(≈0.98) 이상이면 매 틱 vital −= r. **thermal→vital도 같은 필드 재사용**(원인만 다른 drive). 엔진: `nextVital = regenVital − Σ starveDrain`(부호대칭). **off-lever = byte-identical:** 모든 기존 drive는 `VitalDrain=0` ⇒ 무변(hunger에 저작한 종만 opt-in). **사망 경로:** world `applyAnimalCombat`의 기존 `Vital≤0` 체크(자기-own vital 소진 = 비전투사인 ⇒ 전투사망은 `applyAnimalAttack`에서 이미 라벨·제거)를 **`causeStarvation`**으로 라벨(관측용). **B3 대칭:** 포식자·초식 동일(포식자도 아사 ⇒ 항온기 없이 상한 조절).

### 5 Phases (각 독립 shippable + 결정성 골든; respawn 하네스는 P_fa4c에서만 강등)

- **P_fa4a — 냄새 추적 불완전성 (PD1)** ✅ **SHIPPED (2026-07-16, 커밋 039c268 + 골든 d9bba67).** `engine/fauna` 소비자(`steerFull`, `baseSteerDir` 뒤)에서 homing 채널(seek:food/seek:prey/feed:carrion)에 confidence-blend: `dir = headingDir·(1−conf) + scentDir·conf`, `conf = g·I/(1+g·I)`, `g = Rules.ScentAcuity(sp,ctx)` (§6, keenness gain — **RESOLVE 프리뷰의 `I/(I+acuity)`는 부호 역전 오류**였고 실제는 keenness-gain form이라 "wolf가 더 잘 추적"=큰 g가 성립). flee/wary(steer-away)·early-warning은 **미열화**(prey 경보는 coexistence에 유리). **off-lever=byte-identical:** `scent_acuity` 미저작 OR g≤0 ⇒ blend skip(정확 scent Dir). **PD1b(§6 스탯 합성):** `scent_acuity` = §6(Perception) — 신규 `Perception` capability 스탯(stats.yaml, D10) + 5 homing 종(deer/rabbit/goat/wolf/bear) objects.yaml 저작(bear 최고예민 superb nose, wolf 다음, herbivore 중간; fish=homing 없음→미저작) + 픽스처 템플릿 Perception 값(rabbit_meadow/arena/village/minimal). fish/prey 등 미저작 종·agent 골든 **무변**(agent는 런타임 Gen 샘플링 없음→스트림 무교란). **respawn ON(하네스), 신규 개체군 모델 없음(조향만, 가역).** 배선: `SpeciesRule.ScentAcuity`·`speciesData.scentAcuity`·`Rules.ScentAcuity`·config `parseOptionalFaunaFormula`·objects.schema.json `scent_acuity`. **검증:** arena hunt-success **12.8%→10.3%**(밴드 내, engage=68/kills=7, prey 34→35 안정 — 포식자 faint-scent 추적 소폭 저하=refugia 방향) + 단위테스트 `scent_acuity_test.go`(neutral 정확homing·faint→wander·keener-nose·§6 Perception 합성·off-lever byte-id·결정성) + 전체 1309 통과. SPEC: `fauna/SPEC-steering.md`(§Scent-tracking confidence)·`fauna/SPEC.md`. **잔여(P_fa4a 튜닝):** g·Perception 값 전부 UNTUNED placeholder; 라이브 refugia 육안검증(장기 트레이스에서 피식자 은신처 형성·포식자 전맵-훑기 소멸)은 배포 후.
- **P_fa4b — flora 고갈 + 아사 (PD2·PD3)** ✅ **SHIPPED (2026-07-18).**
  - **검증(관측):** ① **arena**(3000틱, 강제조우) hunt-success 22.2%(밴드 내 <50%) + **starvation 219**(deer92·rabbit125·**wolf2**) → 지표를 predation-only로 필터(전투사 라벨=predation, 아사=starvation 분리). ② **peaceful meadow**(rabbit_meadow 500², 3000틱): 개체군 **안정·멸종 없음**(respawn 하네스), **초식 대부분 생존**(deer1·rabbit6 아사 — 방목으로 연명), **포식자 아사**(wolf14·bear7) — D4 대칭 아사가 "불멸 포식자 항온기"를 대체(수용 충족). ③ 단위/통합 GREEN: fauna `starve_test.go`(포화→출혈·유예(fed regen)·θ경계·off-lever byte-id·clamp·결정성)·flora `graze_test.go`(crop·availability cap·no-op·ID타겟)·world `starve_graze_test.go`(depletion+availability recovery+off-lever, 아사사망 cause=starvation) + 전체 스위트 GREEN.
  - **⚠️ 잔여(튜닝, P_fa4c/d):** meadow에서 **predation 0**(포식자가 P_fa4a 냄새 불완전+500² 넓이로 prey를 못 찾아 사냥 전 아사) — 포식-피식 루프가 아직 약함. 이는 phased 설계대로 **P_fa4c(창발 번식+튜닝)에서 루프 창발**로 해소 예정. 포식자 아사 churn 크기는 respawn 템플릿 hunger 초기값+포식자 밀도+scent_acuity에 의존(전부 UNTUNED). r=0.006·θ=0.98·k=0.5 placeholder.
  - **PD2 flora 고갈 (SPEC 상세, 하위 OPEN 확정):** `applyAnimalGraze`가 대상 flora **Plant.Length**(=biomass proxy, 이미 food scent mag=`Length+Width`·yield qty·stage를 구동)를 소모 — 신규 flora API `State.GrazeLength(id, amount)`(in-place, ID-정렬 이진탐색 O(log n), D12). **소모량 §6 shape:** biomass 소비 = 실제 hunger 회복에 비례(conservation) — `graze_depletion_per_hunger`(world-Length/hunger 1.0, balance 상수 D9) × 회복량. **회복=availability 스케일:** `removed = min(mult×k, Length)`, `recovery = removed/k` ⇒ 고갈된 그루(Length↓)는 회복도 ↓(=국소 기근); 건강한 그루는 recovery=mult(현행). **off-lever = byte-identical:** `graze_depletion_per_hunger ≤ 0` ⇒ 소모 skip·recovery=mult(기존 골든 유지). **재성장:** 기존 flora LengthRate가 감소된 Length에서 성장 재개(graze→고갈→재성장 루프 완결). Width(cover/shade)는 **무변**(방목=식용 높이만 crop, 은신처는 별개). berry_shrub `depletes` 재사용은 **불필요**(Length 소비가 kind-무관 일반 경로). scent mag 감쇠가 **P_fa4a confidence-blend와 결합**(고갈 그루=희미한 냄새=homing 실패=이동압).
  - **PD3 아사 (위 RESOLVE 상세):** `DriveRule.VitalDrain`/`VitalDrainAbove` + `nextVital` 출혈 + `causeStarvation` 라벨. hunger drive에 balance 값 저작(포식자·초식 대칭).
  - **respawn ON(하네스 — 튜닝 중 멸종 방지).** 수용: 과방목 지점에서 초식 아사 관측·먹이부족 시 포식자 아사 관측·대량 동시아사(붕괴) 없음. "공유지 비극" 시나리오(§2 P_fa4 원문) 검증. **잔여:** r·θ·k 전부 UNTUNED placeholder.
- **P_fa4c — 창발 번식(2-parent) + respawn→rescue 강등 (PD4·PD5)** 🔨 **빌드 중 (2026-07-18).** PD4=(b) 2-parent, 하위 PD4-i/ii/iii RESOLVED. **active mate-seeking**(PD4 (b) "신규 mate-seeking action" + PD4-i 근접질의 ⇒ 능동 탐색). **이 phase에서 루프가 창발이 됨.** 대형 phase라 **서브-phase 분할** 착지:
  - **P_fa4c-1 (age/성숙 leaf) ✅ 빌드됨(2026-07-18):** 신규 `Animal.Age`(float64, 매 틱 world가 +FaunaDT) + `Animal.MateCooldownUntil`(P_fa4c-2용, 함께 추가) + `SpeciesRule.MaturityAge` + §6 파생 operand `maturity = clamp01(Age/MaturityAge)`(AttrOperands 추가, D10, `rules.maturity()`·context 사전계산) — Mate §6·파트너 자격이 읽음. off-lever: maturity_age 미저작(≤0) ⇒ maturity≡1(게이트 중립). **골든 무교란**(Age는 어떤 golden도 digest 안 함, persist는 내부상태 미직렬화라 Vital처럼 무영향) + **행동 중립**(아직 maturity 읽는 §6 없음). 테스트: fauna `maturity_test.go`(age 게이트·clamp·off-lever)·world `age_test.go`(틱당 +FaunaDT). config `maturity_age` 파싱 + schema. age→노화(§7)는 **후행 별개**. **잔여:** maturity_age 값 저작은 P_fa4c-2(Mate action과 함께).
  - **P_fa4c-2 (mate-seeking + 수정) — 🔨 구현 완료·⚠️ 수용 미달 (2026-07-19).** 메커니즘은 전부 동작(단위 6종 GREEN, 전체 1341 통과, 라이브 픽스처에서 창발 번식 실측 확인) **그러나 §5가 c-2 수용 조건으로 요구한 개체수 tripwire가 발동** ⇒ **c-3 착지 전 배포 금지.**
    - **실측(rabbit_meadow 500², 16000틱):** 토끼 15→17→25→32→**71→138→264→1049→2407**(가속), 사슴 8→108, 전체 35→2534. **아사가 상한이 되지 못함**(과방목→기근 되먹임이 이 창에서 안 잡힘). PD4-v를 (c) c-3 이연으로 고른 대가가 정확히 이 형태로 나타난 것 — respawn 하네스는 **floor(재도입)이지 cap이 아니다**. ⇒ **PD4-v 밀도 게이트는 선택이 아니라 c-3의 필수 선행**, 그리고 방목 고갈 상수(k)·hunger rate 재튜닝도 함께 봐야 함.
    - **설계 수정 1건(구현 중 발견, 중요):** PD4-vi ⓒ "상호 합의"를 **"같은 틱에 양쪽 intent가 서로를 지목"** 으로 구현하면 **현실에서 수정이 거의 불가능**하다 — 짝 탐색은 full pipeline에서만 돌고, 초식동물은 대부분 F45 **DORMANT**이며 재중재 phase가 **ID별로 어긋나** 두 개체가 같은 틱에 동시 재중재할 일이 거의 없다(실측: 8000틱, 다수가 Mate 선택, **출생 0**). ⇒ 합의를 **파트너의 커밋된 `CurrentAction`** 으로 읽도록 착지(구애는 "한 틱의 우연"이 아니라 **개체가 놓인 상태**). 일방 성립은 여전히 거부(파트너가 사냥/도주/기아-채집 중이면 무산) ⇒ §6 utility의 hunger/fear 항이 번식률을 실제로 통제한다는 성질은 보존.
    - **PD4-iv 세부 착지(결정에 명시 없던 부분 — 사람 확인 요망):** "±변이"의 크기 출처를 **부모 자신의 간격**으로 잡았다: stat별 seeded `lean∈[0,1)`로 두 부모를 블렌드한 뒤 `inherit`만큼 평균 쪽으로 당김(`child = mean + (raw−mean)(1−inherit)`). 새 σ 상수를 발명하지 않고, 결과가 **항상 두 부모 사이에 갇혀** 범위를 벗어날 수 없다. 이게 중요한 이유: **fauna 픽스처 스탯은 0–100 스케일인데 공유 `stats.yaml` 레지스트리 range는 [0,1]** 이라(실측), 레지스트리 Min/Max나 `gen.sd`를 크기 근거로 쓰면 동물에게는 전부 오작동한다. `inherit`은 이 변경이 **첫 소비자**(그동안 로드·검증만 되고 아무도 안 읽었음).
    - 배선: fauna `TagSteerMate`/`mate.go`(mateTarget=combatTarget 형 argmin)/`Intent.MateWith`/`Rules.MateCooldown`·`IsCourting`; world `fauna_mate_apply.go`(pass-2 cross-write, 저-ID 1회 수정, `spawnOffspring`, `blendParentStats`); config `mate_cooldown`+`seek:mate` 태그 허용; content `Mate` 액션(produces 없음 ⇒ agent planner 무영향)+5종 `maturity_age`/`mate_cooldown`/Mate utility. off-lever: Mate 미저작 ⇒ 무번식·바이트동일.
    - **아래는 원래 계획 형태(참고).** **선행 실측(2026-07-19, 착수 가능성 조사):** ✅ 재사용 확인 — `combatTarget`(combat.go:28)이 이미 spatial 이웃 질의 + (거리,ID) tie-break argmin이라 PD4-i 파트너 탐색의 **정확한 템플릿**; `AnimalBorn` 이벤트는 이미 존재(respawn.go:84 emit); `applyAnimal*` 동사 패턴(graze/drink/feed)이 `applyAnimalMate`의 자리; c-1이 `Age`·`MateCooldownUntil`·`MaturityAge`·`maturity` operand를 이미 깔아둠. ❌ 없음 — `seek:mate` steer 태그(현 태그는 food/water/prey뿐), 동종 밀도 operand, fauna `carrying_capacity`. content `Mate` action(`seek:mate` steer tag) + §6 utility(`maturity*(1-hunger) - fear*2` 류). fauna Step: seek:mate ⇒ Spatial로 최근접 **자격 파트너**(동종·mature·비쿨다운·Mate행동) 질의(combatTarget 재사용, PD4-i) → 그쪽으로 steer. world apply `applyAnimalMate`: 파트너가 conception 사거리 내면 seeded roll → **즉시 offspring 1**(저-ID 부모가 spawn near, PD4-iii) + 부모 stat `inherit` 혼합(PD4-iv) + `carrying_capacity` §6 게이트(PD4b) + 양쪽 `MateCooldownUntil` 설정(EngageCooldownUntil 패턴, SPEC 상세) + `AnimalBorn` emit. cadence=flora 번식과 동일(PD4c). off-lever: Mate action/maturity_age 미저작 ⇒ 무번식(byte-id).
  - **P_fa4c-3 (respawn→rescue, PD5) — ⚠️ 이제 c-2의 폭주를 막는 선행이기도 함.** ①**PD4-v 밀도 게이트**(c-2 tripwire 발동으로 필수화) ②respawn top-up을 **rescue floor**로 강등 — 종이 `rescue_threshold`(예 ≤2) 이하일 때만 낮은 rate 재도입(현 `respawn_target`/`respawn_cadence` 재해석). 평시 완전 창발, 근사멸종만 확률 구제.
  - **수용:** 장기(수천~수만 틱) 종별 개체군 **밴드 안정**(항온기 없이) + 근사멸종 확률적 복구 관측 + 포식자 폭주 없음 + 신규 offspring가 성숙→번식(세대 순환). 신규 풀-월드 장기 개체군 밴드 회귀 테스트 추가.
