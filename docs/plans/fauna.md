...
> **게이트 상태(개체군 동역학):** PD1=**RESOLVED (c) confidence-blend**·PD1a=**RESOLVED (b) 소비자(D12 강제)**·PD1b=**RESOLVED (c) §6 스탯 합성(`scent_acuity`)**; PD2=**RESOLVED (b) flora 고갈**; PD3=**RESOLVED (a) 유예+출혈 (사람 승인 2026-07-17)**; PD4=**RESOLVED (b) 2-parent 짝짓기 (사람 승인 2026-07-18)** — 하위: PD4-i/ii/iii **RESOLVED**(2026-07-18) → **P_fa4c-1 SHIPPED**; **PD4-iv·PD4-v·PD4-vi = OPEN ⇒ P_fa4c-2 착수 차단**(2026-07-19 재확인 — iv는 여태 `rec`였고 RESOLVE된 적 없음; v는 PD4b가 "flora carrying_capacity 재사용"이라 했으나 fauna에 해당 operand·키가 **없음**을 코드로 확인); PD5=**RESOLVED (b) rescue floor**(단 하위 값/배치 OPEN ⇒ P_fa4c-3 차단); PD6=**rec PD1 우선**; PD7=**OPEN(P_fa4d, 선택)**. **P_fa4a·P_fa4b 게이트 전부 RESOLVED ✓ → P_fa4a·P_fa4b SHIPPED(아래 Phases).** 다음 빌드=P_fa4c(PD4-i/ii/iii RESOLVE 후). **핵심 안전 계약: 매 phase는 새 메커니즘 하나만 respawn 하네스에 대고 관측 — "번식 없는 아사"·"은신처 없는 창발"을 구조적으로 회피.**
>
