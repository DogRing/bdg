...
- **PD4 — 창발 번식 (emergent birth — respawn top-up 대체).** *options:* (a) **무성 근접 spawn** 포만+성숙(vital↑) 개체가 slow cadence로 근처에 offspring(drive-gated roll) — **flora 씨앗분산 모델(§1a) 그대로 재사용**(부모근접·seeded·carrying_capacity 가중). (b) 2-parent 짝짓기(신규 mate-seeking action 필요). (c) 에너지예산(PD3c와 결합, 출산이 reserve 소모). rec였던 것은 (a)이나 **RESOLVED: (b) 2-parent 짝짓기 (사람 승인 2026-07-18).** — 더 풍부한 사회적 창발(짝 탐색·만남). **PD4b(carrying capacity):** flora `carrying_capacity` §6 **재사용**(동종밀도 logistic로 출산 억제 — 하드캡 없이 폭주 방지, 이미 RESOLVED). **PD4c(cadence/결정성):** flora와 동일(world per-step fork, `tick%N` bulk, D12).
  - **⚠️ (b)는 그린필드 — `repro_readiness`(F19/F25)는 어휘만 있고 미배선, age/성숙 필드 없음.** 착수 전 아래 하위 게이트 RESOLVE 필요:
    - **PD4-i 짝 탐색:** 준비된 개체가 파트너를 어떻게 찾나? (i) **근접 spatial-hash 질의**(combatTarget 재사용 — 근거리 동종 성숙+준비 최근접, 신규 scent 채널 없음) *rec* · (ii) 신규 `scent:mate` 채널(원거리 유인, scent+P_fa4a 재사용하나 채널/deposit 추가) · (iii) 기존 prey 냄새 재사용(mate·target 혼동). **OPEN.**
    - **PD4-ii 준비/성숙 게이트:** 무엇이 개체를 "번식 준비"로? (i) **신규 `repro_readiness` 어큐뮬레이터 drive**(출생부터 상승=성숙 proxy + 짝짓기 후 리셋=쿨다운, 포만(hunger 낮음) AND-게이트) *rec — drive 기계 재사용* · (ii) 신규 age 필드 · (iii) vital/condition 임계만. **OPEN.**
    - **PD4-iii 수정→offspring:** 준비된 두 파트너가 만나면 (i) **seeded 수정 roll → 즉시 근접 offspring**(D12 per-step fork, 저-ID 부모가 spawn, 양쪽 readiness 리셋) *rec* · (ii) 임신 지연(N틱 후 출산 — pregnancy 필드 필요). **OPEN.**
    - **PD4-iv offspring 스탯:** 부모 stat `inherit` 파라미터로 혼합(평균+변이, stat 시스템 재사용) + `carrying_capacity` §6 게이트(PD4b). *rec — 재사용.* **OPEN(경미).**
