...
> **게이트 상태(개체군 동역학):** PD1=**RESOLVED (c) confidence-blend**·PD1a=**RESOLVED (b) 소비자(D12 강제)**·PD1b=**RESOLVED (c) §6 스탯 합성(`scent_acuity`)**; PD2=**RESOLVED (b) flora 고갈**; PD3=**RESOLVED (a) 유예+출혈 (사람 승인 2026-07-17)**; PD4=**OPEN(P_fa4c: birth trigger; capacity/cadence 재사용=rec)**; PD5=**RESOLVED (b) rescue floor**; PD6=**rec PD1 우선**; PD7=**OPEN(P_fa4d, 선택)**. **P_fa4a·P_fa4b 게이트 전부 RESOLVED ✓ → P_fa4a·P_fa4b SHIPPED(아래 Phases).** 다음 빌드=P_fa4c(PD4 birth trigger RESOLVE 후). **핵심 안전 계약: 매 phase는 새 메커니즘 하나만 respawn 하네스에 대고 관측 — "번식 없는 아사"·"은신처 없는 창발"을 구조적으로 회피.**
>
