...
### 1m. `carrying_capacity`의 온도 계약 — **RESOLVED 2026-07-19 (사람 확정)**
...
- [RESOLVED→(B)] **K의 온도 계약** — options: (A) 현행 유지(관례만) · **(B) 로드 타임 거부** · (C) 배치에 climate 초기 온도(연 중앙선) 공급 · (D) 배치용 수식(`establishment_weight`) 분리. **채택: (B) (사람 확정 2026-07-19).** `buildFloraRules`가 `carrying_capacity`에서 `temperature`/`thermal_stress`를 읽으면 **로드 실패** — 기존 `neighbor_count` 순환 거부와 **같은 자리·같은 선례**. 배치에 climate가 없다는 건 파이프라인의 구조적 사실이므로 대리값으로 덮지 않고 존중한다(대리 온도는 사실처럼 읽히는 허구). 개념 분리도 이쪽이 깔끔하다 — **K = 이 자리가 구조적으로 몇 그루를 지탱하나**(지형·수분) / **suitability = 지금 얼마나 잘 자라나**(기후 포함); shipped 수식들이 이미 그렇게 쓰여 있다. 조용한 오류를 시끄러운 오류로 바꾸는 것도 1l의 fail-loud 페어링과 같은 수(手). 옵션별 장단점·산술 → `docs/decisions/flora-thermal-comfort.md`.
- **계약:** `carrying_capacity`는 **temperature-free**(강제, 관례 아님). 에러 메시지가 "온도 반응은 `suitability`에 쓰라"고 **행선지를 지목**한다(차단이 아니라 우회 안내). **(C)는 문서화된 탈출구** — 온도 의존 K가 실제로 필요해지면 이 검사를 풀고 같은 변경에서 배치 온도를 공급한다(두 단계는 충돌 없이 연속).
