...
Physical setup that complements `docs/core/architecture.md` (logical module deps).
...
- Engine/platform modules are implemented **leaf-first via the spec-first agent flow** — see
  `docs/core/architecture.md` (order) and each `backend/**/SPEC.md`.
...
make test-update       # regenerate golden snapshots — REVIEW the diff (docs/core/testing.md)
```
...
- SSE/frontend is added later as its own Service; it tails the Redis events stream (`docs/core/data-contracts.md` §4).