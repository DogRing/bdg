...
2. **`docs/architecture.md`** — the module dependency DAG, the import sets, and the **leaf-first build
   order** (§5). You implement in that order; never out of order.
3. **`docs/glossary.md`** — every identifier uses these names. No naming drift.
...
6. For the env/fauna wiring modules: the relevant **Tier-2 plan** in `docs/` (`docs/fauna.md`,
   `docs/flora.md`, `docs/climate.md`, `docs/world-integration.md`) — these hold the binding resolutions
...
- **The Open-Question gate (the biggest risk for an external agent).** Each SPEC has an **Open
  ...
  *"Inventing a mechanism is a defect, not initiative"* (CLAUDE.md). Most leaf SPECs here have no
  ...
  `docs/activation-gate.md`** (G1–G17, with options + recommendations + a `DECISION:` line each); an
...
Leaf-first (from `docs/architecture.md §5`). **Implement the ⬜ rows, in order.**
...
1. **Activation** — place species in the fixture, turn climate ON (wind/°C), turn flora `Rules` ON, and
   re-baseline the affected goldens. **Human-gated** — see **`docs/activation-gate.md`** Part 1 (G1–G5).
...
3. **End-to-end scenario tests** — the FA1–FA8 ecosystem scenarios in `docs/scenarios-world.md` are
